...
namespace VisionSystemUserInterface
{
    ...
    public class Global
    {
        ...
        public static VisionSystemUserInterface.RejectsView RejectsViewWindow;//REJECTS VIEW窗口
        ...
        public static void _GetControllerApplicationVersion()
        {
            ...
            if (null != VisionSystem.Camera && 0 < VisionSystem.Camera.Length)//有效
            {
                ...
                for (i = 0; i < VisionSystem.Camera.Length; i++)
                {
    ...
    public enum CommunicationInstructionType : byte
    {
        ...
        //REJECTS VIEW页面操作
        Rejects_Update = 9,//查询图像，格式：
        Rejects_ClickListItem = 10,//查看列表图像，格式：
        Rejects_BackupAll = 11,//备份所有图像，格式：
        ...
        Rejects_ClearAll = 12,//清除图像，格式：
        //服务端->客户端：指令类型 + 相机类型数据
        //客户端->服务端：指令类型 + 相机类型数据 + 图像清除结果（1，成功；0，失败）
        ...
        QualityCheck_LoadReject_Update = 27,//查询图像
        ...
        //服务端->客户端：指令类型 + 相机类型数据 + 统计数据类型（0，最新统计数据；1，指定统计数据） + 班次索引（非0） + 统计数据开始结束时间
        //客户端->服务端：指令类型 + 相机类型数据 + 统计数据类型（0，当前班；1，历史班） + 班次索引（非0） + 统计数据开始结束时间 + 统计数据（品牌名称（包括品牌长度） + 已检测数量 + 合格数量 + 剔除数量 + 工具数量 + 工具统计信息）

        Statistics_UpdateSelectedRecordData = 101,//STATISTICS页面，更新当前选择的统计数据
        //服务端->客户端：指令类型 + 相机类型数据 + 班次索引（非0） + 统计数据开始结束时间 + 剔除图像对应的工具索引值（从0开始） + 剔除图像对应的工具中的索引值（从0开始） + 图像尺寸类型数据
        //客户端->服务端：指令类型 + 相机类型数据 + 统计数据类型（0，当前班；1，历史班） + 班次索引（非0） + 统计数据开始结束时间 + 统计数据（品牌名称（包括品牌长度） + 已检测数量 + 合格数量 + 剔除数量 + 工具数量 + 工具统计信息）

        Statistics_ClickRejectsListItem = 102,//STATISTICS页面，查看剔除图像，格式：
        //服务端->客户端：指令类型 + 相机类型数据 + 班次索引（非0） + 统计数据开始结束时间 + 剔除图像对应的工具索引值（从0开始） + 剔除图像对应的工具中的索引值（从0开始） + 图像尺寸类型数据
        //客户端->服务端：指令类型 + 相机类型数据 + 班次索引（非0） + 统计数据开始结束时间 + 剔除图像对应的工具索引值（从0开始） + 剔除图像对应的工具中的索引值（从0开始） + 图像尺寸类型数据 + 图像宽度数据 + 图像高度数据 + 图像信息数据 + 图像数据
