...
namespace VisionSystemUserInterface
{
    public partial class Work : Template
    {
        ...
        public void _SetWindow()
        {
            switch (Global.CurrentInterface)
            {
                ...
                case ApplicationInterface.RejectsView://REJECTS VIEW页面                            
                    ...
                    {
                        Global.RejectsViewWindow.Visible = false;//隐藏
                    }
        ...
        //----------------------------------------------------------------------
        // 功能说明：点击【REJECTS】按钮事件，打开REJECTS VIEW窗口
        // 输入参数：1.sender：控件自身的引用
        //         2.e：事件传递的参数
        // 输出参数：无
        // 返回值：无
        //----------------------------------------------------------------------
        private void workControl_Rejects_Click(object sender, EventArgs e)
        {
            if (0 <= Global.VisionSystem.Work.SelectedCameraIndex)//当前选择的相机在相机数组中的索引值有效
            {
                Global.RejectsViewWindow._SetWindow();//设置窗口
            }
        }
        ...
        private void workControl_PasswordEnter(object sender, EventArgs e)
        {
            ...
            else if (3 == workControl.PasswordType)//维护密码
            {
                ...
                Global.RejectsViewWindow.Dispose();//REJECTS VIEW窗口
            ...
            else if (VisionSystemCommunicationLibrary.Ethernet.NetDataType.Instruction == serverData.DataInfo.DataType)//接收的数据为指令，设备信息外的一般指令
            {
                switch ((CommunicationInstructionType)serverData.ReceivedData[serverData.DataInfo.InstructionIndex])
                {
                    ...
                    case CommunicationInstructionType.Load:
                        ...
                        for (Int32 j = 0; j < Global.VisionSystem.Camera.Length; j++)
                        {
                            if (sControllerENGName == Global.VisionSystem.Camera[j].ControllerENGName)//有效
                            {
                        ...
                        try
                        {
                            switch (Global.CurrentInterface)
                            {
                                ...
                                case ApplicationInterface.DevicesSetup://DEVICES SETUP页面
                                    ...
                                    //未完成文件发送
                                    //服务端->客户端（文件）：指令类型 + 相机类型数据 + 设置为的IP地址 + 设置为的相机类型数据 + 设置为的端口 + 设置为的模式 + 设置为的相机故障标记 + 设置为的相机检测使能 + 设置为的相机旋转角度 + 相机颜色 + 相机分辨率 + 是否为串口 + 烟支排列类型 + 设置为的相机数据截取区域缩放 + 设置为的相机数据截取区域缩放后是否居中 + 设置为的相机数据截取区域粘贴区域 + 设置为的相机数据截取区域 + 设置为的相机缺陷图像缓存 + 镜像标记 + 文件索引值（从0开始） + 文件
                                    //客户端->服务端（数据）：指令类型 + 相机类型数据 + 设置为的IP地址 + 设置为的相机类型数据 + 设置为的端口 + 文件索引值（从0开始） + 文件传输状态（1，文件发送中；2，文件发送完成） + 文件接收结果（1，成功；0，失败）

                                    //完成文件发送
                                    //服务端->客户端：指令类型 + 相机类型数据 + 设置为的IP地址 + 设置为的相机类型数据 + 设置为的端口 + 设置为的模式 + 设置为的相机故障标记 + 设置为的相机旋转角度 + 相机颜色 + 相机分辨率 + 是否为串口 + 烟支排列类型 + 设置为的相机数据截取区域缩放 + 设置为的相机数据截取区域缩放后是否居中 + 设置为的相机数据截取区域粘贴区域 + 设置为的相机数据截取区域 + 设置为的相机缺陷图像缓存 + 镜像标记
                                    ...
                                    Int32 ImageNumberTotal_DevicesSetupConfigDevice = 0;//设置为的相机缺陷图像缓存
                                    ...
                                    if (1 == iValue_2_DevicesSetupConfigDevice)//文件发送中
                                    {
                                        ...
                                        else//发送文件
                                        {
                                            ...
                                            Global.WorkWindow._SendCommand_Value(CommunicationInstructionType.DevicesSetup_ConfigDevice, Cameratype_DevicesSetupConfigDevice, iCameraIndex_DevicesSetupConfigDevice, iSelectedIPAddress_DevicesSetupConfigDevice, SelectedCameraType_DevicesSetupConfigDevice, byteSelectedPort_DevicesSetupConfigDevice, CameraChooseState_DevicesSetupConfigDevice, CameraFaultState_DevicesSetupConfigDevice, CheckEnable_DevicesSetupConfigDevice, CameraAngle_DevicesSetupConfigDevice, VideoColor_DevicesSetupConfigDevice, VideoResolution_DevicesSetupConfigDevice, IsSerialPort_DevicesSetupConfigDevice, TobaccoSortType_DevicesSetupConfigDevice, BitmapResize_DevicesSetupConfigDevice, BitmapCenter_DevicesSetupConfigDevice, BitmapAxis_DevicesSetupConfigDevice, BitmapArea_DevicesSetupConfigDevice, ImageNumberTotal_DevicesSetupConfigDevice, CameraFlip_DevicesSetupConfigDevice, sCameraPath, sFileName, iValue_1_DevicesSetupConfigDevice);//发送指令
                                        }
                                    }
                                    else//文件发送完成
                                    {
                    ...
                    case CommunicationInstructionType.DevicesSetup_ConfigImage_Live:
                        ...
                        //客户端->服务端：指令类型 + 相机类型数据 + 图像工具数据（1，包含工具；0，不包含工具） + 图像尺寸类型数据 + 图像宽度数据 + 图像高度数据 + 剔除图像索引值数据 + 剔除图像个数数据 + 剔除图像名称索引值数据 + 图像信息数据 + 图像数据
                        ...
                        Int32 iToolIndex_DevicesSetupConfigImageLive = 0;//工具索引
                        Int32 iRejectImageNumber_DevicesSetupConfigImageLive = 0;//（无意义）
                        Byte[] byteRejectListImage_DevicesSetupConfigImageLive = null;//（无意义）
                        ...
                        _GetInstructionData(serverData, ref Cameratype_DevicesSetupConfigImageLive, ref bViewToolGraphics_DevicesSetupConfigImageLive, ref dImageScale_DevicesSetupConfigImageLive, ref iToolIndex_DevicesSetupConfigImageLive, ref iRejectImageNumber_DevicesSetupConfigImageLive, ref byteRejectListImage_DevicesSetupConfigImageLive, ref iImageDataLength_DevicesSetupConfigImageLive, ref GraphicsInformation_DevicesSetupConfigImageLive, ref imageData_DevicesSetupConfigImageLive,ref imageWidth_DevicesSetupConfigImageLive,ref imageHeight_DevicesSetupConfigImageLive);//解析指令数据
                    ...
                    case CommunicationInstructionType.Work:
                        ...
                        Int32 iRejectImageIndex_Work = 0;//（无意义）
                        Int32 iRejectImageNumber_Work = 0;//（无意义）
                        Byte[] byteRejectListImage_Work = null;//（无意义）
                        ...
                        _GetInstructionData(serverData, ref Cameratype_Work, ref bViewToolGraphics_Work, ref dImageScale_Work, ref iRejectImageIndex_Work, ref iRejectImageNumber_Work, ref byteRejectListImage_Work, ref iImageDataLength_Work, ref GraphicsInformation_Work, ref imageData_Work,ref imageWidth_Work,ref imageHeight_Work);//解析指令数据
                    ...
                    case CommunicationInstructionType.Live:
                        ...
                        Int32 iRejectImageIndex_Live = 0;//（无意义）
                        Int32 iRejectImageNumber_Live = 0;//（无意义）
                        Byte[] byteRejectListImage_Live = null;//（无意义）
                        ...
                        _GetInstructionData(serverData, ref Cameratype_Live, ref bViewToolGraphics_Live, ref dImageScale_Live, ref iRejectImageIndex_Live, ref iRejectImageNumber_Live, ref byteRejectListImage_Live, ref iImageDataLength_Live, ref GraphicsInformation_Live, ref imageData_Live,ref imageWidth_Live,ref imageHeight_Live);//解析指令数据
                        ...
                        break;
                        //
                    case CommunicationInstructionType.Rejects_Update:

                        //REJECTS VIEW页面操作，查询图像，格式：
                        //客户端->服务端：指令类型 + 相机类型数据 + 图像工具数据（1，包含工具；0，不包含工具） + 图像尺寸类型数据 + 图像宽度数据 + 图像高度数据 + 剔除图像索引值数据 + 剔除图像个数数据 + 剔除图像名称索引值数据 + 图像信息数据 + 图像数据

                        VisionSystemClassLibrary.Enum.CameraType Cameratype_RejectsUpdate = VisionSystemClassLibrary.Enum.CameraType.Camera_1;
                        Boolean bViewToolGraphics_RejectsUpdate = true;
                        Double dImageScale_RejectsUpdate = 0.0;
                        Int32 iImageDataLength_RejectsUpdate = 0;
                        Int32 iRejectImageIndex_RejectsUpdate = 0;
                        Int32 iRejectImageNumber_RejectsUpdate = 0;
                        Byte[] byteRejectListImage_RejectsUpdate = null;
                        VisionSystemClassLibrary.Struct.ImageInformation GraphicsInformation_RejectsUpdate = new VisionSystemClassLibrary.Struct.ImageInformation();
                        MemoryStream imageData_RejectsUpdate = new MemoryStream();//获取的图像数据
                        Int32 imageWidth_RejectsUpdate = 0;//获取图像宽度
                        Int32 imageHeight_RejectsUpdate = 0;//获取图像高度

                        _GetInstructionData(serverData, ref Cameratype_RejectsUpdate, ref bViewToolGraphics_RejectsUpdate, ref dImageScale_RejectsUpdate, ref iRejectImageIndex_RejectsUpdate, ref iRejectImageNumber_RejectsUpdate, ref byteRejectListImage_RejectsUpdate, ref iImageDataLength_RejectsUpdate, ref GraphicsInformation_RejectsUpdate, ref imageData_RejectsUpdate,ref imageWidth_RejectsUpdate,ref imageHeight_RejectsUpdate);//解析指令数据
                        ...
                        try
                        {
                            switch (Global.CurrentInterface)
                            {
                                ...
                                case ApplicationInterface.RejectsView://REJECTS VIEW页面                            
                                    ...
                                    //_SendCommand_Image(CommunicationInstructionType.Rejects_Update, Global.VisionSystem.Work.SelectedCameraType, Global.VisionSystem.Work.SelectedCameraIndex);//发送指令

                                    //

                                    Int32 iCameraIndex_RejectsUpdate = _GetSelectedCameraIndex(Cameratype_RejectsUpdate);//相机索引值

                                    //

                                    if (0 < iRejectImageNumber_RejectsUpdate)//存在图像
                                    {
                                        for (Int32 i = byteRejectListImage_RejectsUpdate.Length - 1; i >= 0; i--)
                                        {
                                            if (i >= byteRejectListImage_RejectsUpdate.Length - iRejectImageNumber_RejectsUpdate)//有效图像
                                            {
                                                Global.VisionSystem.Camera[iCameraIndex_RejectsUpdate].Rejects.GraphicsInformation[i].Valid = true;//图像是否有效。true：是；false：否
                                                Global.VisionSystem.Camera[iCameraIndex_RejectsUpdate].Rejects.GraphicsInformation[i].ToolsIndex = byteRejectListImage_RejectsUpdate[i];//图像所属的工具索引值（从0开始）
                                                Global.VisionSystem.Camera[iCameraIndex_RejectsUpdate].Rejects.GraphicsInformation[i].Type = VisionSystemClassLibrary.Enum.ImageType.Error;//图像类型
                                                if (0 <= byteRejectListImage_RejectsUpdate[i])//剔除图像
                                                {
                                                    Global.VisionSystem.Camera[iCameraIndex_RejectsUpdate].Rejects.GraphicsInformation[i].Name = Global.VisionSystem.Camera[iCameraIndex_RejectsUpdate].Tools[byteRejectListImage_RejectsUpdate[i]].Name;//信息名称
                                                }
                                                else//无效
                                                {
                                                    Global.VisionSystem.Camera[iCameraIndex_RejectsUpdate].Rejects.GraphicsInformation[i].Name = "";//信息名称
                                                }
                                            }
                                            else//无效图像
                                            {
                                                Global.VisionSystem.Camera[iCameraIndex_RejectsUpdate].Rejects.GraphicsInformation[i].Valid = false;//图像是否有效。true：是；false：否
                                                Global.VisionSystem.Camera[iCameraIndex_RejectsUpdate].Rejects.GraphicsInformation[i].Name = "";//信息名称
                                            }
                                        }
                                    }
                                    ...
                                    if (0 < iImageDataLength_RejectsUpdate)//图像有效
                                    {
                                        Bitmap bitmap_RejectsUpdate = new Bitmap(imageData_RejectsUpdate);
                                        BitmapData bitmapData_RejectsUpdate = bitmap_RejectsUpdate.LockBits(new Rectangle(0, 0, imageWidth_RejectsUpdate, imageHeight_RejectsUpdate), ImageLockMode.ReadOnly, PixelFormat.Format24bppRgb);
                                        Image<Bgr, Byte> image_RejectsUpdate = new Image<Bgr, Byte>(imageWidth_RejectsUpdate, imageHeight_RejectsUpdate, bitmapData_RejectsUpdate.Stride, bitmapData_RejectsUpdate.Scan0);

                                        Global.VisionSystem.Camera[iCameraIndex_RejectsUpdate].ImageReject = image_RejectsUpdate.Copy();//更新数值

                                        bitmap_RejectsUpdate.UnlockBits(bitmapData_RejectsUpdate);

                                        Boolean bValid_RejectsUpdate = Global.VisionSystem.Camera[iCameraIndex_RejectsUpdate].Rejects.GraphicsInformation[Global.VisionSystem.Camera[iCameraIndex_RejectsUpdate].Rejects.CurrentIndex].Valid;
                                        String sName_RejectsUpdate = Global.VisionSystem.Camera[iCameraIndex_RejectsUpdate].Rejects.GraphicsInformation[Global.VisionSystem.Camera[iCameraIndex_RejectsUpdate].Rejects.CurrentIndex].Name;

                                        Global.VisionSystem.Camera[iCameraIndex_RejectsUpdate].Rejects.GraphicsInformation[Global.VisionSystem.Camera[iCameraIndex_RejectsUpdate].Rejects.CurrentIndex] = GraphicsInformation_RejectsUpdate;//图像信息
                                        Global.VisionSystem.Camera[iCameraIndex_RejectsUpdate].Rejects.GraphicsInformation[Global.VisionSystem.Camera[iCameraIndex_RejectsUpdate].Rejects.CurrentIndex].Valid = bValid_RejectsUpdate;//图像信息
                                        Global.VisionSystem.Camera[iCameraIndex_RejectsUpdate].Rejects.GraphicsInformation[Global.VisionSystem.Camera[iCameraIndex_RejectsUpdate].Rejects.CurrentIndex].Name = sName_RejectsUpdate;//信息名称

                                        Global.VisionSystem.Camera[iCameraIndex_RejectsUpdate].Rejects.CurrentIndex = (Int16)iRejectImageIndex_RejectsUpdate;//当前索引值

                                        //
                                    }
                                    else//图像无效
                                    {
                                        Global.VisionSystem.Camera[iCameraIndex_RejectsUpdate].ImageReject = null;//更新数值

                                        Global.VisionSystem.Camera[iCameraIndex_RejectsUpdate].Rejects.GraphicsInformation[Global.VisionSystem.Camera[iCameraIndex_RejectsUpdate].Rejects.CurrentIndex] = new VisionSystemClassLibrary.Struct.ImageInformation();//图像信息
                                        Global.VisionSystem.Camera[iCameraIndex_RejectsUpdate].Rejects.GraphicsInformation[Global.VisionSystem.Camera[iCameraIndex_RejectsUpdate].Rejects.CurrentIndex].Valid = false;//图像信息
                                        Global.VisionSystem.Camera[iCameraIndex_RejectsUpdate].Rejects.GraphicsInformation[Global.VisionSystem.Camera[iCameraIndex_RejectsUpdate].Rejects.CurrentIndex].Name = "";//图像信息
                                        Global.VisionSystem.Camera[iCameraIndex_RejectsUpdate].Rejects.GraphicsInformation[Global.VisionSystem.Camera[iCameraIndex_RejectsUpdate].Rejects.CurrentIndex].ValueDisplay = false;//图像信息
                                        Global.VisionSystem.Camera[iCameraIndex_RejectsUpdate].Rejects.GraphicsInformation[Global.VisionSystem.Camera[iCameraIndex_RejectsUpdate].Rejects.CurrentIndex].Type = VisionSystemClassLibrary.Enum.ImageType.Error;//图像信息
                                        Global.VisionSystem.Camera[iCameraIndex_RejectsUpdate].Rejects.GraphicsInformation[Global.VisionSystem.Camera[iCameraIndex_RejectsUpdate].Rejects.CurrentIndex].Value = new Boolean[VisionSystemClassLibrary.Struct.ImageInformation.TotalNumber];//图像信息
                                    }

                                    //

                                    if (!Global.RejectsViewWindow.CustomControl.ClickRejectListItem)//此时需处理点击事件
                                    {
                                        Global.RejectsViewWindow.CustomControl.Invoke(new EventHandler(delegate { Global.RejectsViewWindow.CustomControl._SetImageInformation(iRejectImageIndex_RejectsUpdate, iRejectImageNumber_RejectsUpdate); }));
                                    }
                        ...
                        break;
                        //
                    case CommunicationInstructionType.Rejects_ClickListItem:

                        //REJECTS VIEW页面操作，查看列表图像，格式：
                        //客户端->服务端：指令类型 + 相机类型数据 + 图像工具数据（1，包含工具；0，不包含工具） + 图像尺寸类型数据 + 图像宽度数据 + 图像高度数据 + 剔除图像索引值数据 + 剔除图像个数数据 + 剔除图像名称索引值数据 + 图像信息数据 + 图像数据

                        VisionSystemClassLibrary.Enum.CameraType Cameratype_RejectsClickListItem = VisionSystemClassLibrary.Enum.CameraType.Camera_1;
                        Boolean bViewToolGraphics_RejectsClickListItem = true;
                        Double dImageScale_RejectsClickListItem = 0.0;
                        Int32 iImageDataLength_RejectsClickListItem = 0;
                        Int32 iRejectImageIndex_RejectsClickListItem = 0;
                        Int32 iRejectImageNumber_RejectsClickListItem = 0;
                        Byte[] byteRejectListImage_RejectsClickListItem = null;                
                        VisionSystemClassLibrary.Struct.ImageInformation GraphicsInformation_RejectsClickListItem = new VisionSystemClassLibrary.Struct.ImageInformation();
                        MemoryStream imageData_RejectsClickListItem = new MemoryStream();//获取的图像数据
                        Int32 imageWidth_RejectsClickListItem = 0;//获取图像宽度
                        Int32 imageHeight_RejectsClickListItem = 0;//获取图像高度

                        _GetInstructionData(serverData, ref Cameratype_RejectsClickListItem, ref bViewToolGraphics_RejectsClickListItem, ref dImageScale_RejectsClickListItem, ref iRejectImageIndex_RejectsClickListItem, ref iRejectImageNumber_RejectsClickListItem, ref byteRejectListImage_RejectsClickListItem, ref iImageDataLength_RejectsClickListItem, ref GraphicsInformation_RejectsClickListItem, ref imageData_RejectsClickListItem,ref imageWidth_RejectsClickListItem,ref imageHeight_RejectsClickListItem);//解析指令数据
                        ...
                        try
                        {
                            switch (Global.CurrentInterface)
                            {
                                ...
                                case ApplicationInterface.RejectsView://REJECTS VIEW页面                            
                                    ...
                                    Int32 iCameraIndex_RejectsClickListItem = _GetSelectedCameraIndex(Cameratype_RejectsClickListItem);//相机索引值

                                    //

                                    if (0 < iRejectImageNumber_RejectsClickListItem)//存在图像
                                    {
                                        for (Int32 i = byteRejectListImage_RejectsClickListItem.Length - 1; i >= 0; i--)
                                        {
                                            if (i >= byteRejectListImage_RejectsClickListItem.Length - iRejectImageNumber_RejectsClickListItem)//有效图像
                                            {
                                                Global.VisionSystem.Camera[iCameraIndex_RejectsClickListItem].Rejects.GraphicsInformation[i].Valid = true;//图像是否有效。true：是；false：否
                                                Global.VisionSystem.Camera[iCameraIndex_RejectsClickListItem].Rejects.GraphicsInformation[i].ToolsIndex = byteRejectListImage_RejectsClickListItem[i];//图像所属的工具索引值（从0开始）
                                                Global.VisionSystem.Camera[iCameraIndex_RejectsClickListItem].Rejects.GraphicsInformation[i].Type = VisionSystemClassLibrary.Enum.ImageType.Error;//图像类型
                                                if (0 <= byteRejectListImage_RejectsClickListItem[i])//剔除图像
                                                {
                                                    Global.VisionSystem.Camera[iCameraIndex_RejectsClickListItem].Rejects.GraphicsInformation[i].Name = Global.VisionSystem.Camera[iCameraIndex_RejectsClickListItem].Tools[byteRejectListImage_RejectsClickListItem[i]].Name;//信息名称
                                                }
                                                else//无效
                                                {
                                                    Global.VisionSystem.Camera[iCameraIndex_RejectsClickListItem].Rejects.GraphicsInformation[i].Name = "";//信息名称
                                                }
                                            }
                                            else//无效图像
                                            {
                                                Global.VisionSystem.Camera[iCameraIndex_RejectsClickListItem].Rejects.GraphicsInformation[i].Valid = false;//图像是否有效。true：是；false：否
                                                Global.VisionSystem.Camera[iCameraIndex_RejectsClickListItem].Rejects.GraphicsInformation[i].Name = "";//信息名称
                                            }
                                        }
                                    }
                                    ...
                                    if (0 < iImageDataLength_RejectsClickListItem)//图像有效
                                    {
                                        Bitmap bitmap_RejectsClickListItem = new Bitmap(imageData_RejectsClickListItem);
                                        BitmapData bitmapData_RejectsClickListItem = bitmap_RejectsClickListItem.LockBits(new Rectangle(0, 0, imageWidth_RejectsClickListItem, imageHeight_RejectsClickListItem), ImageLockMode.ReadOnly, PixelFormat.Format24bppRgb);
                                        Image<Bgr, Byte> image_RejectsClickListItem = new Image<Bgr, Byte>(imageWidth_RejectsClickListItem, imageHeight_RejectsClickListItem, bitmapData_RejectsClickListItem.Stride, bitmapData_RejectsClickListItem.Scan0);

                                        Global.VisionSystem.Camera[iCameraIndex_RejectsClickListItem].ImageReject = image_RejectsClickListItem.Copy();//更新数值

                                        bitmap_RejectsClickListItem.UnlockBits(bitmapData_RejectsClickListItem);

                                        Boolean bValid_RejectsClickListItem = Global.VisionSystem.Camera[iCameraIndex_RejectsClickListItem].Rejects.GraphicsInformation[Global.VisionSystem.Camera[iCameraIndex_RejectsClickListItem].Rejects.CurrentIndex].Valid;
                                        String sName_RejectsClickListItem = Global.VisionSystem.Camera[iCameraIndex_RejectsClickListItem].Rejects.GraphicsInformation[Global.VisionSystem.Camera[iCameraIndex_RejectsClickListItem].Rejects.CurrentIndex].Name;

                                        Global.VisionSystem.Camera[iCameraIndex_RejectsClickListItem].Rejects.GraphicsInformation[Global.VisionSystem.Camera[iCameraIndex_RejectsClickListItem].Rejects.CurrentIndex] = GraphicsInformation_RejectsClickListItem;//图像信息
                                        Global.VisionSystem.Camera[iCameraIndex_RejectsClickListItem].Rejects.GraphicsInformation[Global.VisionSystem.Camera[iCameraIndex_RejectsClickListItem].Rejects.CurrentIndex].Valid = bValid_RejectsClickListItem;//图像信息
                                        Global.VisionSystem.Camera[iCameraIndex_RejectsClickListItem].Rejects.GraphicsInformation[Global.VisionSystem.Camera[iCameraIndex_RejectsClickListItem].Rejects.CurrentIndex].Name = sName_RejectsClickListItem;//信息名称

                                        Global.VisionSystem.Camera[iCameraIndex_RejectsClickListItem].Rejects.CurrentIndex = (Int16)iRejectImageIndex_RejectsClickListItem;//当前索引值

                                        //
                                    }
                                    else//图像无效
                                    {
                                        Global.VisionSystem.Camera[iCameraIndex_RejectsClickListItem].ImageReject = null;//更新数值

                                        Global.VisionSystem.Camera[iCameraIndex_RejectsClickListItem].Rejects.GraphicsInformation[Global.VisionSystem.Camera[iCameraIndex_RejectsClickListItem].Rejects.CurrentIndex] = new VisionSystemClassLibrary.Struct.ImageInformation();//图像信息
                                        Global.VisionSystem.Camera[iCameraIndex_RejectsClickListItem].Rejects.GraphicsInformation[Global.VisionSystem.Camera[iCameraIndex_RejectsClickListItem].Rejects.CurrentIndex].Valid = false;//图像信息
                                        Global.VisionSystem.Camera[iCameraIndex_RejectsClickListItem].Rejects.GraphicsInformation[Global.VisionSystem.Camera[iCameraIndex_RejectsClickListItem].Rejects.CurrentIndex].Name = "";//图像信息
                                        Global.VisionSystem.Camera[iCameraIndex_RejectsClickListItem].Rejects.GraphicsInformation[Global.VisionSystem.Camera[iCameraIndex_RejectsClickListItem].Rejects.CurrentIndex].ValueDisplay = false;//图像信息
                                        Global.VisionSystem.Camera[iCameraIndex_RejectsClickListItem].Rejects.GraphicsInformation[Global.VisionSystem.Camera[iCameraIndex_RejectsClickListItem].Rejects.CurrentIndex].Type = VisionSystemClassLibrary.Enum.ImageType.Error;//图像信息
                                        Global.VisionSystem.Camera[iCameraIndex_RejectsClickListItem].Rejects.GraphicsInformation[Global.VisionSystem.Camera[iCameraIndex_RejectsClickListItem].Rejects.CurrentIndex].Value = new Boolean[VisionSystemClassLibrary.Struct.ImageInformation.TotalNumber];//图像信息
                                    }

                                    //

                                    Global.RejectsViewWindow.CustomControl.ClickRejectListItem = false;

                                    Global.RejectsViewWindow.CustomControl.Invoke(new EventHandler(delegate { Global.RejectsViewWindow.CustomControl._SetImageInformation(iRejectImageIndex_RejectsClickListItem, iRejectImageNumber_RejectsClickListItem); }));
                        ...
                        break;
                        //
                    case CommunicationInstructionType.Rejects_BackupAll:

                        //REJECTS VIEW页面操作，备份所有图像，格式：
                        //客户端->服务端：指令类型 + 相机类型数据 + 图像工具数据（1，包含工具；0，不包含工具） + 图像尺寸类型数据 + 图像宽度数据 + 图像高度数据 + 剔除图像索引值数据 + 剔除图像个数数据 + 剔除图像名称索引值数据 + 图像信息数据 + 图像数据

                        VisionSystemClassLibrary.Enum.CameraType Cameratype_RejectsBackupAll = VisionSystemClassLibrary.Enum.CameraType.Camera_1;
                        Boolean bViewToolGraphics_RejectsBackupAll = true;
                        Double dImageScale_RejectsBackupAll = 0.0;
                        Int32 iImageDataLength_RejectsBackupAll = 0;
                        Int32 iRejectImageIndex_RejectsBackupAll = 0;//（无意义）
                        Int32 iRejectImageNumber_RejectsBackupAll = 0;//（无意义）
                        Byte[] byteRejectListImage_RejectsBackupAll = null;//（无意义）
                        VisionSystemClassLibrary.Struct.ImageInformation GraphicsInformation_RejectsBackupAll = new VisionSystemClassLibrary.Struct.ImageInformation();
                        MemoryStream imageData_RejectsBackupAll = new MemoryStream();//获取的图像数据
                        Int32 imageWidth_RejectsBackupAll = 0;//获取图像宽度
                        Int32 imageHeight_RejectsBackupAll = 0;//获取图像高度

                        _GetInstructionData(serverData, ref Cameratype_RejectsBackupAll, ref bViewToolGraphics_RejectsBackupAll, ref dImageScale_RejectsBackupAll, ref iRejectImageIndex_RejectsBackupAll, ref iRejectImageNumber_RejectsBackupAll, ref byteRejectListImage_RejectsBackupAll, ref iImageDataLength_RejectsBackupAll, ref GraphicsInformation_RejectsBackupAll, ref imageData_RejectsBackupAll,ref imageWidth_RejectsBackupAll,ref imageHeight_RejectsBackupAll);//解析指令数据

                        Int32 iCameraIndex_RejectsBackupAll = _GetSelectedCameraIndex(Cameratype_RejectsBackupAll);//相机索引值

                        if (0 < iImageDataLength_RejectsBackupAll)//图像有效
                        {
                            try
                            {
                                switch (Global.CurrentInterface)
                                {
                                    ...
                                    case ApplicationInterface.RejectsView://REJECTS VIEW页面                            
                                        ...
                                        Bitmap bitmap_RejectsBackupAll = new Bitmap(imageData_RejectsBackupAll);
                                        BitmapData bitmapData_RejectsBackupAll = bitmap_RejectsBackupAll.LockBits(new Rectangle(0, 0, imageWidth_RejectsBackupAll, imageHeight_RejectsBackupAll), ImageLockMode.ReadOnly, PixelFormat.Format24bppRgb);
                                        Image<Bgr, Byte> image_RejectsBackupAll = new Image<Bgr, Byte>(imageWidth_RejectsBackupAll, imageHeight_RejectsBackupAll, bitmapData_RejectsBackupAll.Stride, bitmapData_RejectsBackupAll.Scan0);

                                        Global.VisionSystem.Camera[iCameraIndex_RejectsBackupAll].ImageReject = image_RejectsBackupAll.Copy();//更新数值

                                        bitmap_RejectsBackupAll.UnlockBits(bitmapData_RejectsBackupAll);

                                        Global.VisionSystem.Camera[iCameraIndex_RejectsBackupAll].Rejects.GraphicsInformation[iRejectImageIndex_RejectsBackupAll] = GraphicsInformation_RejectsBackupAll;//图像信息

                                        //

                                        Global.RejectsViewWindow.CustomControl.Invoke(new EventHandler(delegate { Global.RejectsViewWindow.CustomControl._BackupImage(true); }));
                            ...
                            try
                            {
                                switch (Global.CurrentInterface)
                                {
                                    ...
                                    case ApplicationInterface.RejectsView://REJECTS VIEW页面                            
                                        ...
                                        Global.VisionSystem.Camera[iCameraIndex_RejectsBackupAll].ImageReject = null;//更新数值

                                        Global.VisionSystem.Camera[iCameraIndex_RejectsBackupAll].Rejects.GraphicsInformation[iRejectImageIndex_RejectsBackupAll] = new VisionSystemClassLibrary.Struct.ImageInformation();//图像信息
                                        Global.VisionSystem.Camera[iCameraIndex_RejectsBackupAll].Rejects.GraphicsInformation[iRejectImageIndex_RejectsBackupAll].Valid = false;//图像信息
                                        Global.VisionSystem.Camera[iCameraIndex_RejectsBackupAll].Rejects.GraphicsInformation[iRejectImageIndex_RejectsBackupAll].Name = "";//图像信息
                                        Global.VisionSystem.Camera[iCameraIndex_RejectsBackupAll].Rejects.GraphicsInformation[iRejectImageIndex_RejectsBackupAll].ValueDisplay = false;//图像信息
                                        Global.VisionSystem.Camera[iCameraIndex_RejectsBackupAll].Rejects.GraphicsInformation[iRejectImageIndex_RejectsBackupAll].Type = VisionSystemClassLibrary.Enum.ImageType.Error;//图像信息
                                        Global.VisionSystem.Camera[iCameraIndex_RejectsBackupAll].Rejects.GraphicsInformation[iRejectImageIndex_RejectsBackupAll].Value = new Boolean[VisionSystemClassLibrary.Struct.ImageInformation.TotalNumber];//图像信息

                                        //

                                        Global.RejectsViewWindow.CustomControl.Invoke(new EventHandler(delegate { Global.RejectsViewWindow.CustomControl._BackupImage(false); }));
                        ...
                        break;
                        //
                    case CommunicationInstructionType.Rejects_ClearAll:

                        //REJECTS VIEW页面，清除图像，格式：
                        //客户端->服务器：指令类型 + 相机类型数据 + 图像清除结果（1，成功；0，失败）

                        VisionSystemClassLibrary.Enum.CameraType Cameratype_RejectsClearAll = VisionSystemClassLibrary.Enum.CameraType.Camera_1;//相机类型数据
                        Int32 iValue_1_RejectsClearAll = 0;//图像清除结果（1，成功；0，失败）

                        _GetInstructionData(serverData, ref Cameratype_RejectsClearAll, ref iValue_1_RejectsClearAll);//解析指令数据

                        Int32 iCameraIndex_RejectsClearAll = _GetSelectedCameraIndex(Cameratype_RejectsClearAll);//相机索引值

                        //

                        for (Int32 i = Global.VisionSystem.Camera[iCameraIndex_RejectsClearAll].Rejects.ImageNumberTotal - 1; i >= 0; i--)
                        {
                            Global.VisionSystem.Camera[iCameraIndex_RejectsClearAll].Rejects.GraphicsInformation[i].Valid = false;//图像是否有效。true：是；false：否
                            Global.VisionSystem.Camera[iCameraIndex_RejectsClearAll].Rejects.GraphicsInformation[i].ToolsIndex = -1;//图像所属的工具索引值（从0开始）
                            Global.VisionSystem.Camera[iCameraIndex_RejectsClearAll].Rejects.GraphicsInformation[i].Name = "";//信息名称
                        }

                        Global.VisionSystem.Camera[iCameraIndex_RejectsClearAll].ImageReject = null;//更新数值
                        ...
                        try
                        {
                            switch (Global.CurrentInterface)
                            {
                                ...
                                case ApplicationInterface.RejectsView://REJECTS VIEW页面                            
                                    ...
                                    Global.RejectsViewWindow.CustomControl.Invoke(new EventHandler(delegate { Global.RejectsViewWindow.CustomControl._ClearAllImagesFinish(Convert.ToBoolean(iValue_1_RejectsClearAll)); }));//操作完成
                    ...
                    case CommunicationInstructionType.TolerancesSettings_Rejects:
                        ...
                        Int32 iRejectImageIndex_TolerancesSettingsRejects = 0;//（无意义）
                        Int32 iRejectImageNumber_TolerancesSettingsRejects = 0;//（无意义）
                        Byte[] byteRejectListImage_TolerancesSettingsRejects = null;//（无意义）
                            ...
                            switch (Global.CurrentInterface)
                            {
                                ...
                                case ApplicationInterface.TolerancesSettings://TOLERANCES SETTINGS页面
                                    ...
                                    if (0 < iImageDataLength_TolerancesSettingsRejects)//图像有效
                                    {
                                        ...
                                        Global.VisionSystem.Camera[iCameraIndex_TolerancesSettingsRejects].Rejects.GraphicsInformation[Global.VisionSystem.Camera[iCameraIndex_TolerancesSettingsRejects].Rejects.ImageNumberTotal - 1] = GraphicsInformation_TolerancesSettingsRejects;//图像信息
                                        ...
                                        {
                                            Global.VisionSystem.Camera[iCameraIndex_TolerancesSettingsRejects].Rejects.GraphicsInformation[Global.VisionSystem.Camera[iCameraIndex_TolerancesSettingsRejects].Rejects.ImageNumberTotal - 1].Name = Global.VisionSystem.Camera[iCameraIndex_TolerancesSettingsRejects].Tools[GraphicsInformation_TolerancesSettingsRejects.ToolsIndex].Name;//信息名称
                                        }
                                        else//无效
                                        {
                                            Global.VisionSystem.Camera[iCameraIndex_TolerancesSettingsRejects].Rejects.GraphicsInformation[Global.VisionSystem.Camera[iCameraIndex_TolerancesSettingsRejects].Rejects.ImageNumberTotal - 1].Name = "";//信息名称
                                        }
                                    ...
                                    else//图像无效
                                    {
                                        ...
                                        Global.VisionSystem.Camera[iCameraIndex_TolerancesSettingsRejects].Rejects.GraphicsInformation[Global.VisionSystem.Camera[iCameraIndex_TolerancesSettingsRejects].Rejects.ImageNumberTotal - 1] = new VisionSystemClassLibrary.Struct.ImageInformation();//图像信息
                                        Global.VisionSystem.Camera[iCameraIndex_TolerancesSettingsRejects].Rejects.GraphicsInformation[Global.VisionSystem.Camera[iCameraIndex_TolerancesSettingsRejects].Rejects.ImageNumberTotal - 1].Valid = false;//图像信息
                                        Global.VisionSystem.Camera[iCameraIndex_TolerancesSettingsRejects].Rejects.GraphicsInformation[Global.VisionSystem.Camera[iCameraIndex_TolerancesSettingsRejects].Rejects.ImageNumberTotal - 1].Name = "";//图像信息
                                        Global.VisionSystem.Camera[iCameraIndex_TolerancesSettingsRejects].Rejects.GraphicsInformation[Global.VisionSystem.Camera[iCameraIndex_TolerancesSettingsRejects].Rejects.ImageNumberTotal - 1].ValueDisplay = false;//图像信息
                                        Global.VisionSystem.Camera[iCameraIndex_TolerancesSettingsRejects].Rejects.GraphicsInformation[Global.VisionSystem.Camera[iCameraIndex_TolerancesSettingsRejects].Rejects.ImageNumberTotal - 1].Type = VisionSystemClassLibrary.Enum.ImageType.Error;//图像信息
                                        Global.VisionSystem.Camera[iCameraIndex_TolerancesSettingsRejects].Rejects.GraphicsInformation[Global.VisionSystem.Camera[iCameraIndex_TolerancesSettingsRejects].Rejects.ImageNumberTotal - 1].Value = new Boolean[VisionSystemClassLibrary.Struct.ImageInformation.TotalNumber];//图像信息
                                    }
                    ...
                    case CommunicationInstructionType.TolerancesSettings_Live:
                        ...
                        Int32 iRejectImageIndex_TolerancesSettingsLive = 0;//（无意义）
                        Int32 iRejectImageNumber_TolerancesSettingsLive = 0;//（无意义）
                        Byte[] byteRejectListImage_TolerancesSettingsLive = null;//（无意义）
                        ...
                        _GetInstructionData(serverData, ref Cameratype_TolerancesSettingsLive, ref bViewToolGraphics_TolerancesSettingsLive, ref dImageScale_TolerancesSettingsLive, ref iRejectImageIndex_TolerancesSettingsLive, ref iRejectImageNumber_TolerancesSettingsLive, ref byteRejectListImage_TolerancesSettingsLive, ref iImageDataLength_TolerancesSettingsLive, ref GraphicsInformation_TolerancesSettingsLive, ref imageData_TolerancesSettingsLive,ref imageWidth_TolerancesSettingsLive,ref imageHeight_TolerancesSettingsLive);//解析指令数据
                        ...
                        try
                        {
                            switch (Global.CurrentInterface)
                            {
                                ...
                                case ApplicationInterface.QualityCheck://QUALITY CHECK页面
                                    ...
                                    {
                                        _SendCommand_Image(CommunicationInstructionType.QualityCheck_LiveView, Global.VisionSystem.Work.SelectedCameraType, Global.VisionSystem.Work.SelectedCameraIndex);//发送指令
                                    }
                    ...
                    case CommunicationInstructionType.QualityCheck_LearnSample:
                        ...
                        //客户端->服务端：指令类型 + 相机类型数据 + 图像工具数据（1，包含工具；0，不包含工具） + 图像尺寸类型数据 + 图像宽度数据 + 图像高度数据 + 剔除图像索引值数据 + 剔除图像个数数据 + 剔除图像名称索引值数据 + 图像信息数据 + 图像处理信息（绘图） + 图像数据
                        ...
                        Int32 iToolIndex_QualityCheckLearnSample = 0;//工具索引
                        Int32 iRejectImageNumber_QualityCheckLearnSample = 0;//（无意义）
                        Byte[] byteRejectListImage_QualityCheckLearnSample = null;//（无意义）
                        ...
                        _GetInstructionData(serverData, ref Cameratype_QualityCheckLearnSample, ref bViewToolGraphics_QualityCheckLearnSample, ref dImageScale_QualityCheckLearnSample, ref iToolIndex_QualityCheckLearnSample, ref iRejectImageNumber_QualityCheckLearnSample, ref byteRejectListImage_QualityCheckLearnSample, ref iImageDataLength_QualityCheckLearnSample, ref GraphicsInformation_QualityCheckLearnSample , ref imageData_QualityCheckLearnSample);//解析指令数据
                        ...
                        if (0 < iImageDataLength_QualityCheckLearnSample)//图像有效
                        {
                            ...
                            Global.VisionSystem.Camera[iCameraIndex_QualityCheckLearnSample].Learn = GraphicsInformation_QualityCheckLearnSample;//图像信息
                    ...
                    case CommunicationInstructionType.QualityCheck_LoadSample:
                        ...
                        Int32 iToolIndex_QualityCheckLoadSample = 0;//工具索引
                        Int32 iRejectImageNumber_QualityCheckLoadSample = 0;//（无意义）
                        Byte[] byteRejectListImage_QualityCheckLoadSample = null;//（无意义）
                        ...
                        _GetInstructionData(serverData, ref Cameratype_QualityCheckLoadSample, ref bViewToolGraphics_QualityCheckLoadSample, ref dImageScale_QualityCheckLoadSample, ref iToolIndex_QualityCheckLoadSample, ref iRejectImageNumber_QualityCheckLoadSample, ref byteRejectListImage_QualityCheckLoadSample, ref iImageDataLength_QualityCheckLoadSample, ref GraphicsInformation_QualityCheckLoadSample, ref imageData_QualityCheckLoadSample);//解析指令数据
                        ...
                        //if (0 < iImageDataLength_QualityCheckLoadSample)//图像有效
                        //{
                        //    Global.VisionSystem.Camera[iCameraIndex_QualityCheckLoadSample].ImageLearn = imageData_QualityCheckLoadSample.Copy();//更新数值
                        //    Global.VisionSystem.Camera[iCameraIndex_QualityCheckLoadSample].Learn = GraphicsInformation_QualityCheckLoadSample;//图像信息

                        //    //

                        //    imageData_QualityCheckLoadSample.Dispose();
                        //}
                        //else//图像无效
                        //{
                        //    Global.VisionSystem.Camera[iCameraIndex_QualityCheckLoadSample].ImageLearn = null;//更新数值

                        //    Global.VisionSystem.Camera[iCameraIndex_QualityCheckLoadSample].Learn = new VisionSystemClassLibrary.Struct.ImageInformation();//图像信息
                        //    Global.VisionSystem.Camera[iCameraIndex_QualityCheckLoadSample].Learn.Valid = false;//图像信息
                        //    Global.VisionSystem.Camera[iCameraIndex_QualityCheckLoadSample].Learn.Name = "";//图像信息
                        //    Global.VisionSystem.Camera[iCameraIndex_QualityCheckLoadSample].Learn.ValueDisplay = false;//图像信息
                        //    Global.VisionSystem.Camera[iCameraIndex_QualityCheckLoadSample].Learn.Type = VisionSystemClassLibrary.Enum.ImageType.Error;//图像信息
                        //    Global.VisionSystem.Camera[iCameraIndex_QualityCheckLoadSample].Learn.Value = new Boolean[VisionSystemClassLibrary.Struct.ImageInformation.TotalNumber];//图像信息
                        //}
                    ...
                    case CommunicationInstructionType.QualityCheck_LiveView:
                        ...
                        Int32 iToolIndex_QualityCheckLiveView = 0;//工具索引
                        Int32 iRejectImageNumber_QualityCheckLiveView = 0;//（无意义）
                        Byte[] byteRejectListImage_QualityCheckLiveView = null;//（无意义）
                        ...
                        _GetInstructionData(serverData, ref Cameratype_QualityCheckLiveView, ref bViewToolGraphics_QualityCheckLiveView, ref dImageScale_QualityCheckLiveView, ref iToolIndex_QualityCheckLiveView, ref iRejectImageNumber_QualityCheckLiveView, ref byteRejectListImage_QualityCheckLiveView, ref iImageDataLength_QualityCheckLiveView, ref GraphicsInformation_QualityCheckLiveView, ref imageData_QualityCheckLiveView);//解析指令数据
                        ...
                        break;
                        //
                    case CommunicationInstructionType.QualityCheck_LoadReject_Update:

                        //QUALITY CHECK页面操作，加载模板图像，格式：
                        //客户端->服务端：指令类型 + 相机类型数据 + 图像工具数据（1，包含工具；0，不包含工具） + 图像尺寸类型数据 + 图像宽度数据 + 图像高度数据 + 剔除图像索引值数据 + 剔除图像个数数据 + 剔除图像名称索引值数据 + 图像信息数据 + 图像处理信息（绘图） + 图像数据

                        VisionSystemClassLibrary.Enum.CameraType Cameratype_QualityCheckLoadRejectUpdate = VisionSystemClassLibrary.Enum.CameraType.Camera_1;
                        Boolean bViewToolGraphics_QualityCheckLoadRejectUpdate = true;
                        Double dImageScale_QualityCheckLoadRejectUpdate = 0.0;
                        Int32 iImageDataLength_QualityCheckLoadRejectUpdate = 0;
                        Int32 iRejectImageIndex_QualityCheckLoadRejectUpdate = 0;
                        Int32 iRejectImageNumber_QualityCheckLoadRejectUpdate = 0;
                        Byte[] byteRejectListImage_QualityCheckLoadRejectUpdate = null;
                        VisionSystemClassLibrary.Struct.ImageInformation GraphicsInformation_QualityCheckLoadRejectUpdate = new VisionSystemClassLibrary.Struct.ImageInformation();
                        Image<Bgr, Byte> imageData_QualityCheckLoadRejectUpdate = null;//获取的图像数据

                        _GetInstructionData(serverData, ref Cameratype_QualityCheckLoadRejectUpdate, ref bViewToolGraphics_QualityCheckLoadRejectUpdate, ref dImageScale_QualityCheckLoadRejectUpdate, ref iRejectImageIndex_QualityCheckLoadRejectUpdate, ref iRejectImageNumber_QualityCheckLoadRejectUpdate, ref byteRejectListImage_QualityCheckLoadRejectUpdate, ref iImageDataLength_QualityCheckLoadRejectUpdate, ref GraphicsInformation_QualityCheckLoadRejectUpdate, ref imageData_QualityCheckLoadRejectUpdate);//解析指令数据

                        Int32 iCameraIndex_QualityCheckLoadRejectUpdate = _GetSelectedCameraIndex(Cameratype_QualityCheckLoadRejectUpdate);//相机索引值

                        //

                        if (0 < iRejectImageNumber_QualityCheckLoadRejectUpdate)//存在图像
                        {
                            for (Int32 i = byteRejectListImage_QualityCheckLoadRejectUpdate.Length - 1; i >= 0; i--)
                            {
                                if (i >= byteRejectListImage_QualityCheckLoadRejectUpdate.Length - iRejectImageNumber_QualityCheckLoadRejectUpdate)//有效图像
                                {
                                    Global.VisionSystem.Camera[iCameraIndex_QualityCheckLoadRejectUpdate].Rejects.GraphicsInformation[i].Valid = true;//图像是否有效。true：是；false：否
                                    Global.VisionSystem.Camera[iCameraIndex_QualityCheckLoadRejectUpdate].Rejects.GraphicsInformation[i].ToolsIndex = byteRejectListImage_QualityCheckLoadRejectUpdate[i];//图像所属的工具索引值（从0开始）
                                    if (0 <= byteRejectListImage_QualityCheckLoadRejectUpdate[i])//剔除图像
                                    {
                                        Global.VisionSystem.Camera[iCameraIndex_QualityCheckLoadRejectUpdate].Rejects.GraphicsInformation[i].Name = Global.VisionSystem.Camera[iCameraIndex_QualityCheckLoadRejectUpdate].Tools[byteRejectListImage_QualityCheckLoadRejectUpdate[i]].Name;//信息名称

                                        if (i == iRejectImageIndex_QualityCheckLoadRejectUpdate)
                                        {
                                            GraphicsInformation_QualityCheckLoadRejectUpdate.Name = Global.VisionSystem.Camera[iCameraIndex_QualityCheckLoadRejectUpdate].Tools[byteRejectListImage_QualityCheckLoadRejectUpdate[i]].Name;//信息名称
                                        }
                                    }
                                    else//完好图像
                                    {
                                        Global.VisionSystem.Camera[iCameraIndex_QualityCheckLoadRejectUpdate].Rejects.GraphicsInformation[i].Name = "OK";//信息名称

                                        if (i == iRejectImageIndex_QualityCheckLoadRejectUpdate)
                                        {
                                            GraphicsInformation_QualityCheckLoadRejectUpdate.Name = "OK";//信息名称
                                        }
                                    }
                                }
                                else//无效图像
                                {
                                    Global.VisionSystem.Camera[iCameraIndex_QualityCheckLoadRejectUpdate].Rejects.GraphicsInformation[i].Valid = false;//图像是否有效。true：是；false：否
                                    Global.VisionSystem.Camera[iCameraIndex_QualityCheckLoadRejectUpdate].Rejects.GraphicsInformation[i].Name = "";//信息名称

                                    if (i == iRejectImageIndex_QualityCheckLoadRejectUpdate)
                                    {
                                        GraphicsInformation_QualityCheckLoadRejectUpdate.Name = "";//信息名称
                                    }
                                }
                            }
                        }
                        ...
                        if (0 < iImageDataLength_QualityCheckLoadRejectUpdate)//图像有效
                        {
                            Global.VisionSystem.Camera[iCameraIndex_QualityCheckLoadRejectUpdate].ImageReject = imageData_QualityCheckLoadRejectUpdate.Copy();//更新数值

                            Boolean bValid_QualityCheckLoadRejectUpdate = Global.VisionSystem.Camera[iCameraIndex_QualityCheckLoadRejectUpdate].Rejects.GraphicsInformation[Global.VisionSystem.Camera[iCameraIndex_QualityCheckLoadRejectUpdate].Rejects.CurrentIndex].Valid;
                            String sName_QualityCheckLoadRejectUpdate = Global.VisionSystem.Camera[iCameraIndex_QualityCheckLoadRejectUpdate].Rejects.GraphicsInformation[Global.VisionSystem.Camera[iCameraIndex_QualityCheckLoadRejectUpdate].Rejects.CurrentIndex].Name;

                            Global.VisionSystem.Camera[iCameraIndex_QualityCheckLoadRejectUpdate].Rejects.GraphicsInformation[Global.VisionSystem.Camera[iCameraIndex_QualityCheckLoadRejectUpdate].Rejects.CurrentIndex] = GraphicsInformation_QualityCheckLoadRejectUpdate;//图像信息
                            Global.VisionSystem.Camera[iCameraIndex_QualityCheckLoadRejectUpdate].Rejects.GraphicsInformation[Global.VisionSystem.Camera[iCameraIndex_QualityCheckLoadRejectUpdate].Rejects.CurrentIndex].Valid = bValid_QualityCheckLoadRejectUpdate;//图像信息
                            Global.VisionSystem.Camera[iCameraIndex_QualityCheckLoadRejectUpdate].Rejects.GraphicsInformation[Global.VisionSystem.Camera[iCameraIndex_QualityCheckLoadRejectUpdate].Rejects.CurrentIndex].Name = sName_QualityCheckLoadRejectUpdate;//信息名称

                            Global.VisionSystem.Camera[iCameraIndex_QualityCheckLoadRejectUpdate].Rejects.CurrentIndex = (Int16)iRejectImageIndex_QualityCheckLoadRejectUpdate;//当前索引值

                            //

                            imageData_QualityCheckLoadRejectUpdate.Dispose();
                        }
                        else//图像无效
                        {
                            Global.VisionSystem.Camera[iCameraIndex_QualityCheckLoadRejectUpdate].ImageReject = null;//更新数值

                            Global.VisionSystem.Camera[iCameraIndex_QualityCheckLoadRejectUpdate].Rejects.GraphicsInformation[Global.VisionSystem.Camera[iCameraIndex_QualityCheckLoadRejectUpdate].Rejects.CurrentIndex] = new VisionSystemClassLibrary.Struct.ImageInformation();//图像信息
                            Global.VisionSystem.Camera[iCameraIndex_QualityCheckLoadRejectUpdate].Rejects.GraphicsInformation[Global.VisionSystem.Camera[iCameraIndex_QualityCheckLoadRejectUpdate].Rejects.CurrentIndex].Valid = false;//图像信息
                            Global.VisionSystem.Camera[iCameraIndex_QualityCheckLoadRejectUpdate].Rejects.GraphicsInformation[Global.VisionSystem.Camera[iCameraIndex_QualityCheckLoadRejectUpdate].Rejects.CurrentIndex].Name = "";//图像信息
                            Global.VisionSystem.Camera[iCameraIndex_QualityCheckLoadRejectUpdate].Rejects.GraphicsInformation[Global.VisionSystem.Camera[iCameraIndex_QualityCheckLoadRejectUpdate].Rejects.CurrentIndex].ValueDisplay = false;//图像信息
                            Global.VisionSystem.Camera[iCameraIndex_QualityCheckLoadRejectUpdate].Rejects.GraphicsInformation[Global.VisionSystem.Camera[iCameraIndex_QualityCheckLoadRejectUpdate].Rejects.CurrentIndex].Type = VisionSystemClassLibrary.Enum.ImageType.Error;//图像信息
                            Global.VisionSystem.Camera[iCameraIndex_QualityCheckLoadRejectUpdate].Rejects.GraphicsInformation[Global.VisionSystem.Camera[iCameraIndex_QualityCheckLoadRejectUpdate].Rejects.CurrentIndex].Value = new Boolean[VisionSystemClassLibrary.Struct.ImageInformation.TotalNumber];//图像信息
                        }
                        ...
                        try
                        {
                            switch (Global.CurrentInterface)
                            {
                                ...
                                case ApplicationInterface.QualityCheck://QUALITY CHECK页面
                                    //
                                    Global.QualityCheckWindow.CustomControl.Invoke(new EventHandler(delegate { Global.QualityCheckWindow.CustomControl._UpdateInterface(GraphicsInformation_QualityCheckLoadRejectUpdate, 3, iRejectImageIndex_QualityCheckLoadRejectUpdate); }));
                                    ...
                                    if (Global.QualityCheckWindow.CustomControl.LoadRejectSelected)//【LOAD REJECT】按下
                                    {
                                        _SendCommand_Image(CommunicationInstructionType.QualityCheck_LoadReject_Update, Global.VisionSystem.Work.SelectedCameraType, Global.VisionSystem.Work.SelectedCameraIndex);//发送指令                                    
                                    }
                    ...
                    case CommunicationInstructionType.QualityCheck_LoadReject_Click:
                        ...
                        Boolean bViewToolGraphics_QualityCheckLoadRejectClick = true;
                        Double dImageScale_QualityCheckLoadRejectClick = 0.0;
                        Int32 iImageDataLength_QualityCheckLoadRejectClick = 0;
                        Int32 iRejectImageIndex_QualityCheckLoadRejectClick = 0;
                        Int32 iRejectImageNumber_QualityCheckLoadRejectClick = 0;
                        Byte[] byteRejectListImage_QualityCheckLoadRejectClick = null;
                        ...
                        _GetInstructionData(serverData, ref Cameratype_QualityCheckLoadRejectClick, ref bViewToolGraphics_QualityCheckLoadRejectClick, ref dImageScale_QualityCheckLoadRejectClick, ref iRejectImageIndex_QualityCheckLoadRejectClick, ref iRejectImageNumber_QualityCheckLoadRejectClick, ref byteRejectListImage_QualityCheckLoadRejectClick, ref iImageDataLength_QualityCheckLoadRejectClick, ref GraphicsInformation_QualityCheckLoadRejectClick, ref imageData_QualityCheckLoadRejectClick);//解析指令数据

                        Int32 iCameraIndex_QualityCheckLoadRejectClick = _GetSelectedCameraIndex(Cameratype_QualityCheckLoadRejectClick);//相机索引值

                        //

                        if (0 < iRejectImageNumber_QualityCheckLoadRejectClick)//存在图像
                        {
                            for (Int32 i = byteRejectListImage_QualityCheckLoadRejectClick.Length - 1; i >= 0; i--)
                            {
                                if (i >= byteRejectListImage_QualityCheckLoadRejectClick.Length - iRejectImageNumber_QualityCheckLoadRejectClick)//有效图像
                                {
                                    Global.VisionSystem.Camera[iCameraIndex_QualityCheckLoadRejectClick].Rejects.GraphicsInformation[i].Valid = true;//图像是否有效。true：是；false：否
                                    Global.VisionSystem.Camera[iCameraIndex_QualityCheckLoadRejectClick].Rejects.GraphicsInformation[i].ToolsIndex = byteRejectListImage_QualityCheckLoadRejectClick[i];//图像所属的工具索引值（从0开始）
                                    if (0 <= byteRejectListImage_QualityCheckLoadRejectClick[i])//剔除图像
                                    {
                                        Global.VisionSystem.Camera[iCameraIndex_QualityCheckLoadRejectClick].Rejects.GraphicsInformation[i].Name = Global.VisionSystem.Camera[iCameraIndex_QualityCheckLoadRejectClick].Tools[byteRejectListImage_QualityCheckLoadRejectClick[i]].Name;//信息名称

                                        if (i == iRejectImageIndex_QualityCheckLoadRejectClick)
                                        {
                                            GraphicsInformation_QualityCheckLoadRejectClick.Name = Global.VisionSystem.Camera[iCameraIndex_QualityCheckLoadRejectClick].Tools[byteRejectListImage_QualityCheckLoadRejectClick[i]].Name;//信息名称
                                        }
                                    }
                                    else//完好图像
                                    {
                                        Global.VisionSystem.Camera[iCameraIndex_QualityCheckLoadRejectClick].Rejects.GraphicsInformation[i].Name = "OK";//信息名称

                                        if (i == iRejectImageIndex_QualityCheckLoadRejectClick)
                                        {
                                            GraphicsInformation_QualityCheckLoadRejectClick.Name = "OK";//信息名称
                                        }
                                    }
                                }
                                else//无效图像
                                {
                                    Global.VisionSystem.Camera[iCameraIndex_QualityCheckLoadRejectClick].Rejects.GraphicsInformation[i].Valid = false;//图像是否有效。true：是；false：否
                                    Global.VisionSystem.Camera[iCameraIndex_QualityCheckLoadRejectClick].Rejects.GraphicsInformation[i].Name = "";//信息名称

                                    if (i == iRejectImageIndex_QualityCheckLoadRejectClick)
                                    {
                                        GraphicsInformation_QualityCheckLoadRejectClick.Name = "";//信息名称
                                    }
                                }
                            }
                        }
                        ...
                        if (0 < iImageDataLength_QualityCheckLoadRejectClick)//图像有效
                        {
                            ...
                            Boolean bValid_QualityCheckLoadRejectClick = Global.VisionSystem.Camera[iCameraIndex_QualityCheckLoadRejectClick].Rejects.GraphicsInformation[Global.VisionSystem.Camera[iCameraIndex_QualityCheckLoadRejectClick].Rejects.CurrentIndex].Valid;
                            String sName_QualityCheckLoadRejectClick = Global.VisionSystem.Camera[iCameraIndex_QualityCheckLoadRejectClick].Rejects.GraphicsInformation[Global.VisionSystem.Camera[iCameraIndex_QualityCheckLoadRejectClick].Rejects.CurrentIndex].Name;

                            Global.VisionSystem.Camera[iCameraIndex_QualityCheckLoadRejectClick].Rejects.GraphicsInformation[Global.VisionSystem.Camera[iCameraIndex_QualityCheckLoadRejectClick].Rejects.CurrentIndex] = GraphicsInformation_QualityCheckLoadRejectClick;//图像信息
                            Global.VisionSystem.Camera[iCameraIndex_QualityCheckLoadRejectClick].Rejects.GraphicsInformation[Global.VisionSystem.Camera[iCameraIndex_QualityCheckLoadRejectClick].Rejects.CurrentIndex].Valid = bValid_QualityCheckLoadRejectClick;//图像信息
                            Global.VisionSystem.Camera[iCameraIndex_QualityCheckLoadRejectClick].Rejects.GraphicsInformation[Global.VisionSystem.Camera[iCameraIndex_QualityCheckLoadRejectClick].Rejects.CurrentIndex].Name = sName_QualityCheckLoadRejectClick;//信息名称

                            Global.VisionSystem.Camera[iCameraIndex_QualityCheckLoadRejectClick].Rejects.CurrentIndex = (Int16)iRejectImageIndex_QualityCheckLoadRejectClick;//当前索引值
                        ...
                        else//图像无效
                        {
                            ...
                            Global.VisionSystem.Camera[iCameraIndex_QualityCheckLoadRejectClick].Rejects.GraphicsInformation[Global.VisionSystem.Camera[iCameraIndex_QualityCheckLoadRejectClick].Rejects.CurrentIndex] = new VisionSystemClassLibrary.Struct.ImageInformation();//图像信息
                            Global.VisionSystem.Camera[iCameraIndex_QualityCheckLoadRejectClick].Rejects.GraphicsInformation[Global.VisionSystem.Camera[iCameraIndex_QualityCheckLoadRejectClick].Rejects.CurrentIndex].Valid = false;//图像信息
                            Global.VisionSystem.Camera[iCameraIndex_QualityCheckLoadRejectClick].Rejects.GraphicsInformation[Global.VisionSystem.Camera[iCameraIndex_QualityCheckLoadRejectClick].Rejects.CurrentIndex].Name = "";//图像信息
                            Global.VisionSystem.Camera[iCameraIndex_QualityCheckLoadRejectClick].Rejects.GraphicsInformation[Global.VisionSystem.Camera[iCameraIndex_QualityCheckLoadRejectClick].Rejects.CurrentIndex].ValueDisplay = false;//图像信息
                            Global.VisionSystem.Camera[iCameraIndex_QualityCheckLoadRejectClick].Rejects.GraphicsInformation[Global.VisionSystem.Camera[iCameraIndex_QualityCheckLoadRejectClick].Rejects.CurrentIndex].Type = VisionSystemClassLibrary.Enum.ImageType.Error;//图像信息
                            Global.VisionSystem.Camera[iCameraIndex_QualityCheckLoadRejectClick].Rejects.GraphicsInformation[Global.VisionSystem.Camera[iCameraIndex_QualityCheckLoadRejectClick].Rejects.CurrentIndex].Value = new Boolean[VisionSystemClassLibrary.Struct.ImageInformation.TotalNumber];//图像信息
                        }
                        ...
                        try
                        {
                            switch (Global.CurrentInterface)
                            {
                                ...
                                case ApplicationInterface.QualityCheck://QUALITY CHECK页面
                                    ...
                                    Global.QualityCheckWindow.CustomControl.Invoke(new EventHandler(delegate { Global.QualityCheckWindow.CustomControl._UpdateInterface(GraphicsInformation_QualityCheckLoadRejectClick, 3, iRejectImageIndex_QualityCheckLoadRejectClick); }));
                    ...
                    case CommunicationInstructionType.QualityCheck_ManageTools:
                        ...
                        //客户端->服务端：指令类型 + 相机类型数据 + 启用工具标记更新结果（1，成功；0，不成功） + 图像信息数据 + 图像处理信息（绘图）
                    ...
                    case CommunicationInstructionType.QualityCheck_ToolParamter:
                        ...
                        //客户端->服务端：指令类型 + 相机类型数据 + 工具参数更新结果（1，成功；0，不成功） + 图像信息数据 + 图像处理信息（绘图）
                    ...
                    case CommunicationInstructionType.QualityCheck_CurrentTool:
                        ...
                        //客户端->服务端：指令类型 + 相机类型数据 + 当前工具设置结果（1，成功；0，不成功） + 图像信息数据 + 图像处理信息（绘图）
                    ...
                    case CommunicationInstructionType.QualityCheck_WorkArea:
                        ...
                        //客户端->服务端：指令类型 + 相机类型数据 + 设置结果（1，成功；0，不成功） + 图像信息数据 + 图像处理信息（绘图）
                        ...
                        try
                        {
                            switch (Global.CurrentInterface)
                            {
                                ...
                                case ApplicationInterface.RejectsView://REJECTS VIEW页面
                                    ...
                                    if (Global.RejectsViewWindow.CustomControl.SelectedCamera.Type == Cameratype_DeviceStateSynchronization)//当前相机
                                    {
                                        Global.VisionSystem.Work.SelectedCameraType = Cameratype_DeviceStateSynchronization;//当前选中的相机类型
                                        Global.VisionSystem.Work.SelectedCameraIndex = (Int16)_GetSelectedCameraIndex(Cameratype_DeviceStateSynchronization);//当前选中的相机显示控件所对应的相机在相机数组中的索引值
                                        Global.VisionSystem.Camera[Global.VisionSystem.Work.SelectedCameraIndex].Live.CameraSelected = true;//选择
                                    ...
                                    Global.RejectsViewWindow.CustomControl.Invoke(new EventHandler(delegate { Global.RejectsViewWindow.CustomControl._DeviceStateChanged(true); }));//更新页面
                                ...
                                case ApplicationInterface.StatisticsView://STATISTICS VIEW页面
                                    ...
                                    {
                                        _SendCommand_Value(CommunicationInstructionType.Statistics_UpdateSelectedRecordData, Global.VisionSystem.Work.SelectedCameraType, Global.VisionSystem.Work.SelectedCameraIndex, Global.VisionSystem.Shift.DataOfShift.CurrentIndex + 1, Global.VisionSystem.Shift.DataOfShift.CurrentStatisticsInformation.TimeData[0], Global.StatisticsViewWindow.CustomControl.CurrentToolIndex_RejectImage, Global.StatisticsViewWindow.CustomControl.CurrentRejectImageIndex_Tool, 1.0);
                                    }
                    ...
                    case CommunicationInstructionType.Statistics_GetSelectedRecordData:
                        ...
                        //客户端->服务端：指令类型 + 相机类型数据 + 统计数据类型（0，当前班；1，历史班） + 班次索引（从0开始） + 统计数据开始结束时间 + 统计数据（已用时间 + 品牌名称（包括品牌长度） + 已检测数量 + 合格数量 + 剔除数量 + 工具数量 + 工具统计信息）
                        ...
                        _GetInstructionData(serverData, ref Cameratype_StatisticsGetSelectedRecordData, ref iStatisticsDataType_StatisticsGetSelectedRecordData, ref Global.VisionSystem.Shift.DataOfShift.CurrentIndex, ref Global.VisionSystem.Shift.DataOfShift.CurrentStatisticsInformation);//解析指令数据
                        ...
                        try
                        {
                            switch (Global.CurrentInterface)
                            {
                                ...
                                case ApplicationInterface.StatisticsView://STATISTICS页面
                                    ...
                                    Global.WorkWindow._SendCommand_Value(CommunicationInstructionType.Statistics_ClickRejectsListItem, Global.VisionSystem.Work.SelectedCameraType, Global.VisionSystem.Work.SelectedCameraIndex, Global.VisionSystem.Shift.DataOfShift.CurrentIndex + 1, Global.VisionSystem.Shift.DataOfShift.CurrentStatisticsInformation.TimeData[0], Global.StatisticsViewWindow.CustomControl.CurrentToolIndex_RejectImage, Global.StatisticsViewWindow.CustomControl.CurrentRejectImageIndex_Tool, 1.0);
                    ...
                    case CommunicationInstructionType.Statistics_UpdateSelectedRecordData:
                        ...
                        _GetInstructionData(serverData, ref Cameratype_StatisticsUpdateSelectedRecordData, ref iStatisticsDataType_StatisticsUpdateSelectedRecordData, ref Global.VisionSystem.Shift.DataOfShift.CurrentIndex, ref Global.VisionSystem.Shift.DataOfShift.CurrentStatisticsInformation);//解析指令数据
                    ...
                    case CommunicationInstructionType.Statistics_ClickRejectsListItem:
                        ...
                        //客户端->服务端：指令类型 + 相机类型数据 + 班次索引（从0开始） + 统计数据开始结束时间 + 剔除图像对应的工具索引值（从0开始） + 剔除图像对应的工具中的索引值（从0开始） + 图像尺寸类型数据 + 图像宽度数据 + 图像高度数据 + 图像信息数据 + 图像数据
        ...
        private void NetServer_ExceptionHandled(object sender, EventArgs e)
        {
            ...
            if (2 == serverException.Timeout)//超时，无效
            {
                ...
                {
                    for (i = 0; i < Global.VisionSystem.Camera.Length; i++)//遍历
                    {
                    ...
                    try
                    {
                        switch (Global.CurrentInterface)
                        {
                            ...
                            case ApplicationInterface.RejectsView://REJECTS VIEW页面                            
                                ...
                                Global.RejectsViewWindow.CustomControl.Invoke(new EventHandler(delegate { Global.RejectsViewWindow.CustomControl._DeviceStateChanged(false); }));//更新程序页面
        ...
        //         5.iBackupImage_Tool_Index：备份图像索引值（QUALITY CHECK页面为工具索引）
        // 输出参数：无
        // 返回值：无
        //----------------------------------------------------------------------
        public void _SendCommand_Image(CommunicationInstructionType command, VisionSystemClassLibrary.Enum.CameraType Cameratype = VisionSystemClassLibrary.Enum.CameraType.Camera_1, Int32 iDataIndex = 0, Double dImageScale = 1.0, Int32 iBackupImage_Tool_Index = 0)
        {
            try
            {
                switch (command)//指令
                {
                    ...
                    case CommunicationInstructionType.Rejects_Update:
                        ...
                        Byte[] RejectsUpdate_Data = _GenerateInstruction(command, Cameratype, Global.RejectsViewWindow.CustomControl.ViewToolGraphics, dImageScale, Global.RejectsViewWindow.CustomControl.CurrentIndex);//生成指令数据

                        Byte[] RejectsUpdate_ClientIP = IPAddress.Parse(Global.VisionSystem.Camera[iDataIndex].DeviceInformation.IP).GetAddressBytes();//获取相机IP地址

                        Global.NetServer.ClientData[RejectsUpdate_ClientIP[3]]._Send(RejectsUpdate_Data);//发送数据
                        //
                        break;
                        //
                    case CommunicationInstructionType.Rejects_ClickListItem:
                        ...
                        Byte[] RejectsClickListItem_Data = _GenerateInstruction(command, Cameratype, Global.RejectsViewWindow.CustomControl.ViewToolGraphics, dImageScale, Global.RejectsViewWindow.CustomControl.CurrentIndex);//生成指令数据

                        Byte[] RejectsClickListItem_ClientIP = IPAddress.Parse(Global.VisionSystem.Camera[iDataIndex].DeviceInformation.IP).GetAddressBytes();//获取相机IP地址

                        Global.NetServer.ClientData[RejectsClickListItem_ClientIP[3]]._Send(RejectsClickListItem_Data);//发送数据
                        //
                        break;
                        //
                    case CommunicationInstructionType.Rejects_BackupAll:
                        ...
                        Byte[] BackupAll_Data = _GenerateInstruction(command, Cameratype, true, dImageScale, iBackupImage_Tool_Index);//生成指令数据

                        Byte[] BackupAll_ClientIP = IPAddress.Parse(Global.VisionSystem.Camera[iDataIndex].DeviceInformation.IP).GetAddressBytes();//获取相机IP地址

                        Global.NetServer.ClientData[BackupAll_ClientIP[3]]._Send(BackupAll_Data);//发送数据
                        //
                        break;
                        //
                    case CommunicationInstructionType.TolerancesSettings_Rejects:
                    ...
                    case CommunicationInstructionType.QualityCheck_LoadSample:
                        ...
                        //QUALITY CHECK页面操作，查看实时图像，格式：
                        //服务端->客户端：指令类型 + 相机类型数据 + 图像工具数据（1，包含工具；0，不包含工具） + 图像尺寸类型数据 + 工具索引

                        Byte[] QualityCheckLoadSample_Data = _GenerateInstruction(command, Cameratype, false, dImageScale, iBackupImage_Tool_Index);//生成指令数据
                    ...
                    case CommunicationInstructionType.QualityCheck_LoadReject_Click:
                        ...
                        Byte[] QualityCheckLoadRejectClick_Data = _GenerateInstruction(command, Cameratype, false, dImageScale, Global.QualityCheckWindow.CustomControl.RejectImageIndex);//生成指令数据

                        Byte[] QualityCheckLoadRejectClick_ClientIP = IPAddress.Parse(Global.VisionSystem.Camera[iDataIndex].DeviceInformation.IP).GetAddressBytes();//获取相机IP地址

                        Global.NetServer.ClientData[QualityCheckLoadRejectClick_ClientIP[3]]._Send(QualityCheckLoadRejectClick_Data);//发送数据
                        //
                        break;
                        //
                    case CommunicationInstructionType.QualityCheck_LoadReject_Update:
                        ...
                        Byte[] QualityCheckLoadRejectUpdate_Data = _GenerateInstruction(command, Cameratype, true, dImageScale, Global.QualityCheckWindow.CustomControl.RejectImageIndex);//生成指令数据

                        Byte[] QualityCheckLoadRejectUpdate_ClientIP = IPAddress.Parse(Global.VisionSystem.Camera[iDataIndex].DeviceInformation.IP).GetAddressBytes();//获取相机IP地址

                        Global.NetServer.ClientData[QualityCheckLoadRejectUpdate_ClientIP[3]]._Send(QualityCheckLoadRejectUpdate_Data);//发送数据
                        //
                        break;
                        //
                    case CommunicationInstructionType.DevicesSetup_ConfigImage_Live:
                        ...
                        Byte[] DevicesSetupConfigImageLive_Data = _GenerateInstruction(command, Cameratype, false, dImageScale, iBackupImage_Tool_Index);//生成指令数据
        ...
        //         4.dImageScale：图像尺寸数据
        //         5.iToolIndex：工具索引
        //         6.iImageType：图像类型。取值范围：1，在线；2，学习；3，剔除
        //         7.iRejectImageIndex：剔除图像索引值（从0开始；非剔除图像时，不使用该值）
        // 输出参数：无
        // 返回值：无
        //----------------------------------------------------------------------
        public void _SendCommand_Image_Learn(CommunicationInstructionType command, VisionSystemClassLibrary.Enum.CameraType Cameratype, Int32 iDataIndex, Double dImageScale, Int32 iToolIndex, Int32 iImageType, Int32 iRejectImageIndex = 0)
        {
            try
            {
                switch (command)//指令
                {
                    case CommunicationInstructionType.QualityCheck_LearnSample:
                        ...
                        //QUALITY CHECK页面操作，自学习阈值或更新，格式：
                        //服务端->客户端：指令类型 + 相机类型数据 + 图像工具数据（1，包含工具；0，不包含工具） + 图像尺寸类型数据 + 工具索引 + 图像类型（1，在线；2，学习；3，剔除） + 剔除图像索引

                        Byte[] QualityCheckLearnSample_Data = _GenerateInstruction(command, Cameratype, false, dImageScale, iToolIndex, iImageType, iRejectImageIndex);//生成指令数据
            ...
            try
            {
                switch (command)//指令
                {
                    ...
                    case CommunicationInstructionType.DevicesSetup_ParameterSettings:
                        ...
                        Byte[] ParameterSettings_Data = _GenerateInstruction(command, Cameratype, Global.VisionSystem.Camera[iDataIndex].DeviceParameter.Parameter);//生成指令数据
                    ...
                    case CommunicationInstructionType.Rejects_ClearAll:
                        ...
                        //REJECTS VIEW页面操作，清除图像，格式：
                        //服务端->客户端：指令类型 + 相机类型数据

                        Byte[] RejectsClearAll_Data = _GenerateInstruction(command, Cameratype);//生成指令数据

                        Byte[] RejectsClearAll_ClientIP = IPAddress.Parse(Global.VisionSystem.Camera[iDataIndex].DeviceInformation.IP).GetAddressBytes();//获取相机IP地址

                        Global.NetServer.ClientData[RejectsClearAll_ClientIP[3]]._Send(RejectsClearAll_Data);//发送数据
                        //
                        break;
                        //
                    case CommunicationInstructionType.TolerancesSettings_Graphs:
                    ...
                    case CommunicationInstructionType.QualityCheck_ManageTools:
                        ...
                        //QUALITY CHECK页面操作，工具管理，格式：
                        //服务端->客户端：指令类型 + 相机类型数据 + 启用工具标记 + 当前工具索引 + 图像类型（1，在线；2，学习；3，剔除） + 剔除图像索引

                        Byte[] QualityCheckManageTools_Data = _GenerateInstruction(command, Cameratype, Global.VisionSystem.Camera[iDataIndex].Tools, iValue_1, iValue_2, iValue_3);//生成指令数据
                    ...
                    case CommunicationInstructionType.QualityCheck_ToolParamter:
                        ...
                        //QUALITY CHECK页面操作，工具参数，格式：
                        //服务端->客户端：指令类型 + 相机类型数据 + 工具索引 + 图像类型（1，在线；2，学习；3，剔除） + 剔除图像索引 + 工具参数

                        Byte[] QualityCheckToolParamter_Data = _GenerateInstruction(command, Cameratype, iValue_1, iValue_2, iValue_3, Global.VisionSystem.Camera[iDataIndex].Tools[iValue_1].Arithmetic);//生成指令数据
                    ...
                    case CommunicationInstructionType.QualityCheck_CurrentTool:
                        ...
                        //QUALITY CHECK页面操作，当前工具设置，格式：
                        //服务端->客户端：指令类型 + 相机类型数据 + 工具索引 + 图像类型（1，在线；2，学习；3，剔除） + 剔除图像索引

                        Byte[] QualityCheckCurrentTool_Data = _GenerateInstruction(command, Cameratype, iValue_1, iValue_2, iValue_3);//生成指令数据
                    ...
                    case CommunicationInstructionType.QualityCheck_WorkArea:
                        ...
                        //服务端->客户端：指令类型 + 相机类型数据 + 工具索引 + 图像类型（1，在线；2，学习；3，剔除） + 剔除图像索引 + 工作区域横坐标 + 工作区域纵坐标 + 工作区域宽度 + 工作区域高度

                        Byte[] QualityCheckWorkArea_Data = _GenerateInstruction(command, Cameratype, iValue_1, iValue_2, iValue_3, iValue_4, iValue_5, iValue_6, iValue_7);//生成指令数据
                ...
                switch (command)//指令
                {
                    case CommunicationInstructionType.DevicesSetup_ConfigDevice:
                        ...
                        else//发送文件
                        {
                            ...
                            //未完成文件发送
                            //服务端->客户端（文件）：指令类型 + 相机类型数据 + 设置为的IP地址 + 设置为的相机类型数据 + 设置为的端口 + 设置为的模式 + 设置为的相机故障标记 + 设置为的相机旋转角度 + 相机颜色 + 相机分辨率 + 是否为串口 + 烟支排列类型 + 设置为的相机数据截取区域缩放 + 设置为的相机数据截取区域缩放后是否居中 + 设置为的相机数据截取区域粘贴区域 + 设置为的相机数据截取区域 + 设置为的相机缺陷图像缓存 + 镜像标记 + 文件序号（从0开始） + 文件

                            DevicesSetupConfigDevice_Data = _GenerateInstruction(command, Cameratype, iSelectedIPAddress, SelectedCameratype, byteSelectedPort, CameraChooseState, uCameraFaultState, bCheckEnable, bCameraAngle, vVideoColor, vVideoResolution, bSerialPort, tTobaccoSortType, bBitmapResize, bBitmapCenter, bBitmapAxis, rBitmapArea, iImageNumberTotal, bCameraFlip, iFileIndex);//生成指令数据
        ...
        // 返回值：无
        //----------------------------------------------------------------------
        public void _SendCommand_Value(CommunicationInstructionType command, VisionSystemClassLibrary.Enum.CameraType Cameratype, Int32 iDataIndex, Int32 iValue_1, Int32 iValue_2, VisionSystemClassLibrary.Struct.ShiftTime shifttime)
        {
            try
            {
                switch (command)//指令
                {
                    case CommunicationInstructionType.Statistics_GetSelectedRecordData:
                        ...
                        //STATISTICS页面，获取当前选择的统计数据，格式：    
                        //服务端->客户端：指令类型 + 相机类型数据 + 统计数据类型（0，最新统计数据；1，指定统计数据） + 班次索引（从0开始） + 统计数据开始结束时间

                        Byte[] StatisticsGetSelectedRecordData_Data = _GenerateInstruction(command, Cameratype, iValue_1, iValue_2, shifttime);//生成指令数据
        ...
        // 返回值：无
        //----------------------------------------------------------------------
        public void _SendCommand_Value(CommunicationInstructionType command, VisionSystemClassLibrary.Enum.CameraType Cameratype, Int32 iDataIndex, Int32 iValue_1, VisionSystemClassLibrary.Struct.ShiftTime shifttime, Int32 iValue_2, Int32 iValue_3, Double dValue)
        {
            try
            {
                switch (command)//指令
                {
                    case CommunicationInstructionType.Statistics_UpdateSelectedRecordData:
                        ...
                        //STATISTICS页面，更新当前选择的统计数据，格式：    
                        //服务端->客户端：指令类型 + 相机类型数据 + 班次索引（从0开始） + 统计数据开始结束时间 + 剔除图像对应的工具索引值（从0开始） + 剔除图像对应的工具中的索引值（从0开始） + 图像尺寸类型数据

                        Byte[] StatisticsUpdateSelectedRecordData_Data = _GenerateInstruction(command, Cameratype, iValue_1, shifttime, iValue_2, iValue_3, dValue);//生成指令数据
                    ...
                    case CommunicationInstructionType.Statistics_ClickRejectsListItem:
                        ...
                        Byte[] StatisticsClickRejectsListItem_Data = _GenerateInstruction(command, Cameratype, iValue_1, shifttime, iValue_2, iValue_3, dValue);//生成指令数据
            ...
            try
            {
                switch (Global.CurrentInterface)
                {
                    case ApplicationInterface.Work://WORK页面
                        ...
                        for (i = iCameraIndex; i < Global.VisionSystem.Camera.Length; i++)//遍历相机
                        {
                        ...
                        if (i >= Global.VisionSystem.Camera.Length && 0 != Global.VisionSystem.Camera.Length)//继续遍历
                        {
        ...
        private void _UpdateTitleBar()
        {
            ...
            if (i < Global.VisionSystem.Camera.Length)//存在未更新、关闭或打开的相机
            {
            ...
            if (bUpdate)//更新按钮
            {
                ...
                Global.RejectsViewWindow.TitleBar.Invoke(new EventHandler(delegate { Global.RejectsViewWindow.TitleBar.StateShow = TitleBar.StateShow; }));//REJECTS VIEW页面
        ...
        private Int32 _CheckConnectedClient(VisionSystemClassLibrary.Enum.CameraType Cameratype)
        {
            ...
            for (i = 0; i < Global.VisionSystem.Camera.Length; i++)//遍历已接纳相机
            {
            ...
            if (i < Global.VisionSystem.Camera.Length)//选中，存在
            {
                iReturn = 1;
            }
            else//其它
                ...
                if (null != Global.VisionSystem.Camera)
                {
                    for (i = 0; i < Global.VisionSystem.Camera.Length; i++)//遍历已接纳相机
                    {
                        if (Cameratype == Global.VisionSystem.Camera[i].Type)//目标相机
                        {
                            ...
                            for (Byte j = 0; j < Global.VisionSystem.Camera.Length; j++)//遍历已接纳相机
                            {
        ...
        public String _GetCameraIPAddress(VisionSystemClassLibrary.Enum.CameraType Cameratype)
        {
            ...
            for (int i = 0; i < Global.VisionSystem.Camera.Length; i++)//遍历相机
            {
                if (Cameratype == Global.VisionSystem.Camera[i].Type)//目标相机
                {
        ...
        private void _SetFaultMessage(Int32 iIndex)
        {
            if (Global.VisionSystem.Shift.ShiftState)//使能班次
            {
                if (0 >= Global.VisionSystem.Work.ConnectedCameraNumber)//不存在相机连接
                {
                    ...
                    Global.RejectsViewWindow.TitleBar.Invoke(new EventHandler(delegate { Global.RejectsViewWindow.TitleBar._ResetFaultMessage(); }));//REJECTS VIEW页面
                ...
                else//存在相机连接
                {
                    ...
                    Global.RejectsViewWindow.TitleBar.Invoke(new EventHandler(delegate { Global.RejectsViewWindow.TitleBar.FaultExist[iIndex] = false; }));//REJECTS VIEW页面
        ...
        private void _SetFaultMessage(Int32 iCameraIndex, VisionSystemClassLibrary.Struct.FaultMessage faultmessage)
        {
            ...
            Global.RejectsViewWindow.TitleBar.Invoke(new EventHandler(delegate { Global.RejectsViewWindow.TitleBar._SetFaultMessage(iCameraIndex, faultmessage); }));//REJECTS VIEW页面
        ...
        //         18.iImageNumberTotal：选择的相机缺陷图像缓存
        //         19.bCameraFlip：选择的相机镜像标记
        // 输出参数：无
        // 返回值：生成的指令
        //----------------------------------------------------------------------
        private Byte[] _GenerateInstruction(CommunicationInstructionType InstructionType, VisionSystemClassLibrary.Enum.CameraType Cameratype, Int32 iSelectedIPAddress, VisionSystemClassLibrary.Enum.CameraType SelectedCameratype, Byte byteSelectedPort, Int32 CameraChooseState, UInt64 uCameraFaultState, Boolean bCheckEnable, VisionSystemClassLibrary.Enum.CameraRotateAngle bCameraAngle, VisionSystemClassLibrary.Enum.VideoColor vVideoColor, VisionSystemClassLibrary.Enum.VideoResolution vVideoResolution, Boolean bSerialPort, VisionSystemClassLibrary.Enum.TobaccoSortType tTobaccoSortType, Boolean bBitmapResize, Boolean bBitmapCenter, Point pBitmapAxis, Rectangle rBitmapArea, Int32 iImageNumberTotal, VisionSystemClassLibrary.Enum.CameraFlip bCameraFlip)
        {
            //指令类型 + 相机类型数据 + 设置为的IP地址 + 设置为的相机类型数据 + 设置为的端口 + 设置为的模式 + 设置为的相机故障标记 + 设置为的相机旋转角度 + 相机颜色 + 相机分辨率 + 是否为串口 + 烟支排列类型 + 设置为的相机数据截取区域缩放 + 设置为的相机数据截取区域缩放后是否居中 + 设置为的相机数据截取区域粘贴区域 + 设置为的相机数据截取区域 + 设置为的相机缺陷图像缓存 + 镜像标记
            ...
            arrayValue_20.CopyTo(arrayInstructionData, iIndex);//填充待发送数据，设置为的相机缺陷图像缓存
            iIndex += arrayValue_20.Length;
            arrayValue_21.CopyTo(arrayInstructionData, iIndex);//填充待发送数据，设置为的相机镜像标记
        ...
        // 返回值：生成的指令
        //----------------------------------------------------------------------
        private Byte[] _GenerateInstruction(CommunicationInstructionType InstructionType, VisionSystemClassLibrary.Enum.CameraType Cameratype, Int32 iSelectedIPAddress, VisionSystemClassLibrary.Enum.CameraType SelectedCameratype, Byte byteSelectedPort, Int32 CameraChooseState, UInt64 uCameraFaultState, Boolean bCheckEnable, VisionSystemClassLibrary.Enum.CameraRotateAngle bCameraAngle, VisionSystemClassLibrary.Enum.VideoColor vVideoColor, VisionSystemClassLibrary.Enum.VideoResolution vVideoResolution, Boolean bSerialPort, VisionSystemClassLibrary.Enum.TobaccoSortType tTobaccoSortType, Boolean bBitmapResize, Boolean bBitmapCenter, Point pBitmapAxis, Rectangle rBitmapArea, Int32 iImageNumberTotal, VisionSystemClassLibrary.Enum.CameraFlip bCameraFlip, Int32 iFileIndex)
        {
            //指令类型 + 相机类型数据 + 设置为的IP地址 + 设置为的相机类型数据 + 设置为的端口 + 设置为的模式 + 设置为的相机故障标记 + 设置为的相机旋转角度 + 相机颜色 + 相机分辨率 + 是否为串口 + 烟支排列类型 + 设置为的相机数据截取区域缩放 + 设置为的相机数据截取区域缩放后是否居中 + 设置为的相机数据截取区域粘贴区域 + 设置为的相机数据截取区域 + 设置为的相机缺陷图像缓存 + 镜像标记 + 文件索引值（从0开始）
            ...
            Byte[] arrayInstructionData = new Byte[1 + 1 + arrayValue_1.Length + arrayValue_2.Length + arrayValue_3.Length + arrayValue_4.Length + arrayValue_5.Length + arrayValue_6.Length + arrayValue_7.Length + arrayValue_8.Length + arrayValue_9.Length + arrayValue_10.Length + arrayValue_11.Length + arrayValue_12.Length + arrayValue_13.Length + arrayValue_14.Length + arrayValue_15.Length + arrayValue_16.Length + arrayValue_17.Length + arrayValue_18.Length + arrayValue_19.Length + arrayValue_20.Length + arrayValue_21.Length + arrayValue_22.Length];//待发送数据
            ...
            arrayValue_20.CopyTo(arrayInstructionData, iIndex);//填充待发送数据，设置为的相机缺陷图像缓存
            iIndex += arrayValue_20.Length;
            arrayValue_21.CopyTo(arrayInstructionData, iIndex);//填充待发送数据，相机镜像标记
        ...
        //         4.iValue_2：数据
        //         5.iValue_3：数据
        //         6.ArithmeticData：工具参数
        // 输出参数：无
        // 返回值：生成的指令
        //----------------------------------------------------------------------
        private Byte[] _GenerateInstruction(CommunicationInstructionType InstructionType, VisionSystemClassLibrary.Enum.CameraType Cameratype, Int32 iValue_1, Int32 iValue_2, Int32 iValue_3, VisionSystemClassLibrary.Struct.Arithmetic ArithmeticData)
        {
            //指令类型 + 相机类型数据 + 工具索引 + 图像类型（1，在线；2，学习；3，剔除） + 剔除图像索引 + 工具参数
            ...
            ethernetData.Data_3 = BitConverter.GetBytes(iValue_3);//剔除图像索引
        ...
        // 返回值：生成的指令
        //----------------------------------------------------------------------
        private Byte[] _GenerateInstruction(CommunicationInstructionType InstructionType, VisionSystemClassLibrary.Enum.CameraType Cameratype, List<VisionSystemClassLibrary.Class.Tools> ToolsData, Int32 iValue_1, Int32 iValue_2, Int32 iValue_3)
        {
            //指令类型 + 相机类型数据 + 启用工具标记 + 当前工具索引 + 图像类型（1，在线；2，学习；3，剔除） + 剔除图像索引
            ...
            ethernetData.Data_4 = BitConverter.GetBytes(iValue_3);//剔除图像索引
        ...
        // 返回值：生成的指令
        //----------------------------------------------------------------------
        private Byte[] _GenerateInstruction(CommunicationInstructionType InstructionType, VisionSystemClassLibrary.Enum.CameraType Cameratype, Int32 iValue_1, Int32 iValue_2, Int32 iValue_3)
        {
            //指令类型 + 相机类型数据 + 工具索引 + 图像类型（1，在线；2，学习；3，剔除） + 剔除图像索引
            ...
            iIndex += arrayValue_2.Length;
            arrayValue_3.CopyTo(arrayInstructionData, iIndex);//填充待发送数据，剔除图像索引
        ...
        // 返回值：生成的指令
        //----------------------------------------------------------------------
        private Byte[] _GenerateInstruction(CommunicationInstructionType InstructionType, VisionSystemClassLibrary.Enum.CameraType Cameratype, Int32 iValue_1, Int32 iValue_2, Int32 iValue_3, Int32 iValue_4)
        {
            //指令类型 + 相机类型数据 + 工具索引 + 工作区域横坐标 + 工作区域纵坐标 + 工作区域宽度 + 工作区域高度
            ...
            iIndex = 2 + arrayValue_1.Length;
            arrayValue_2.CopyTo(arrayInstructionData, iIndex);//填充待发送数据，工作区域横坐标
            iIndex += arrayValue_2.Length;
            arrayValue_3.CopyTo(arrayInstructionData, iIndex);//填充待发送数据，工作区域纵坐标
            iIndex += arrayValue_3.Length;
            arrayValue_4.CopyTo(arrayInstructionData, iIndex);//填充待发送数据，工作区域宽度
        ...
        // 返回值：生成的指令
        //----------------------------------------------------------------------
        private Byte[] _GenerateInstruction(CommunicationInstructionType InstructionType, VisionSystemClassLibrary.Enum.CameraType Cameratype, Int32 iValue_1, Int32 iValue_2, Int32 iValue_3, Int32 iValue_4, Int32 iValue_5, Int32 iValue_6, Int32 iValue_7)
        {
            //指令类型 + 相机类型数据 + 工具索引 + 图像类型（1，在线；2，学习；3，剔除） + 剔除图像索引 + 工作区域横坐标 + 工作区域纵坐标 + 工作区域宽度 + 工作区域高度
            ...
            Byte[] arrayInstructionData = new Byte[1 + 1 + arrayValue_1.Length + arrayValue_2.Length + arrayValue_3.Length + arrayValue_4.Length + arrayValue_5.Length + arrayValue_6.Length + arrayValue_7.Length];//待发送数据
            ...
            arrayValue_3.CopyTo(arrayInstructionData, iIndex);//填充待发送数据，剔除图像索引
            iIndex += arrayValue_3.Length;
            arrayValue_4.CopyTo(arrayInstructionData, iIndex);//填充待发送数据，工作区域横坐标
            iIndex += arrayValue_4.Length;
            arrayValue_5.CopyTo(arrayInstructionData, iIndex);//填充待发送数据，工作区域纵坐标
            iIndex += arrayValue_5.Length;
            arrayValue_6.CopyTo(arrayInstructionData, iIndex);//填充待发送数据，工作区域宽度
            iIndex += arrayValue_6.Length;
            arrayValue_7.CopyTo(arrayInstructionData, iIndex);//填充待发送数据，工作区域高度
        ...
        //         5.iImageIndex：图像索引值数据
        // 输出参数：无
        // 返回值：生成的指令
        //----------------------------------------------------------------------
        private Byte[] _GenerateInstruction(CommunicationInstructionType InstructionType, VisionSystemClassLibrary.Enum.CameraType Cameratype, Boolean bViewToolGraphics, Double dImageScale, Int32 iImageIndex = 0)
        {
            //指令类型 + 相机类型数据 + 图像工具数据（1，包含工具；0，不包含工具） + 图像尺寸类型数据 + 图像索引值数据
            ...
            ethernetData.Data_3 = BitConverter.GetBytes(iImageIndex);//图像索引值数据
        ...
        // 返回值：生成的指令
        //----------------------------------------------------------------------
        private Byte[] _GenerateInstruction(CommunicationInstructionType InstructionType, VisionSystemClassLibrary.Enum.CameraType Cameratype, Boolean bViewToolGraphics, Double dImageScale, Int32 iToolIndex, Int32 iImageType, Int32 iRejectImageIndex)
        {
            //指令类型 + 相机类型数据 + 图像工具数据（1，包含工具；0，不包含工具） + 图像尺寸类型数据 + 工具索引 + 图像类型（1，在线；2，学习；3，剔除） + 剔除图像索引
            ...
            ethernetData.Data_3 = BitConverter.GetBytes(iToolIndex);//工具索引

            ethernetData.Data_4 = BitConverter.GetBytes(iImageType);//图像类型。取值范围：1，在线；2，学习；3，剔除

            ethernetData.Data_5 = BitConverter.GetBytes(iRejectImageIndex);//剔除图像索引值（从0开始；非剔除图像时，不使用该值）
        ...
        private void _threadGetFileData(object objectServerData)
        {
            ...
            System.Array.Copy(ServerData.ReceivedData, ServerData.DataInfo.FileNameIndex, byteFileName, 0, ServerData.DataInfo.FileNameLength);
            ...
            System.Array.Copy(ServerData.ReceivedData, ServerData.DataInfo.InstructionIndex, Load_Data, 0, ServerData.DataInfo.InstructionLength);
        ...
        //         18.iImageNumberTotal：设置为的相机缺陷图像缓存
        //         19.bCameraFlip：设置为的相机镜像标记
        //         20.iValue_1：文件索引值（从0开始）
        //         21.iValue_2：文件传输状态（1，文件发送中；2，文件发送完成）
        //         22.iValue_3：文件接收结果（1，成功；0，失败），配置结果（1，成功；0，失败）
        // 输出参数：无
        // 返回值：生成的指令
        //----------------------------------------------------------------------
        private void _GetInstructionData(VisionSystemCommunicationLibrary.Ethernet.DataReceivedEventArgs ServerData, ref VisionSystemClassLibrary.Enum.CameraType Cameratype, ref Int32 iSelectedIPAddress, ref VisionSystemClassLibrary.Enum.CameraType SelectedCameraType, ref Byte byteSelectedPort, ref Int32 CameraChooseState, ref UInt64 uCameraFaultState, ref Boolean bCheckEanble, ref VisionSystemClassLibrary.Enum.CameraRotateAngle bCameraAngle, ref VisionSystemClassLibrary.Enum.VideoColor vVideoColor, ref VisionSystemClassLibrary.Enum.VideoResolution vVideoResolution, ref Boolean bSerialPort, ref VisionSystemClassLibrary.Enum.TobaccoSortType tTobaccoSortType, ref Boolean bBitmapResize, ref Boolean bBitmapCenter, ref Point pBitmapAxis, ref Rectangle rBitmapArea, ref Int32 iImageNumberTotal, ref VisionSystemClassLibrary.Enum.CameraFlip bCameraFlip, ref Int32 iValue_1, ref Int32 iValue_2, ref Int32 iValue_3)
        {
            //未完成文件发送
            //指令类型 + 相机类型数据 + 设置为的IP地址 + 设置为的相机类型数据 + 设置为的端口 + 设置为的模式 + 设置为的相机故障标记 + 设置为的相机检测使能 + 设置为的相机旋转角度 + 相机颜色 + 相机分辨率 + 是否为串口 + 烟支排列类型 + 设置为的相机数据截取区域缩放 + 设置为的相机数据截取区域缩放后是否居中 + 设置为的相机数据截取区域粘贴区域 + 设置为的相机数据截取区域 + 设置为的相机缺陷图像缓存 + 镜像标记 + 文件索引值（从0开始） + 文件传输状态（1，文件发送中；2，文件发送完成） + 文件接收结果（1，成功；0，失败）

            //完成文件发送
            //指令类型 + 相机类型数据 + 设置为的IP地址 + 设置为的相机类型数据 + 设置为的端口 + 设置为的模式 + 设置为的相机故障标记 + 设置为的相机检测使能 + 设置为的相机旋转角度 + 相机颜色 + 相机分辨率 + 是否为串口 + 烟支排列类型 + 设置为的相机数据截取区域缩放 + 设置为的相机数据截取区域缩放后是否居中 + 设置为的相机数据截取区域粘贴区域 + 设置为的相机数据截取区域 + 设置为的相机缺陷图像缓存 + 镜像标记 + 文件索引值（从0开始） + 文件传输状态（1，文件发送中；2，文件发送完成） + 配置结果（1，成功；0，失败）
            ...
            iIndex += iValueLength;
            iImageNumberTotal = BitConverter.ToInt32(ServerData.ReceivedData, iIndex);//设置为的相机缺陷图像缓存
            iIndex += iValueLength;
        ...
        //         4.dImageScale：相机类型
        //         5.iRejectImageIndex：剔除图像索引值数据
        //         6.iRejectImageNumber：剔除图像个数数据
        //         7.byteRejectListImage：剔除图像名称索引值数据
        //         8.iImageDataLength：图像数据长度
        //         9.GraphicsInformation：图像信息
        //         10.memoryStream：图像数据
        //         11.iImageWidth：图像宽度
        //         12.iImageHeight：图像高度
        // 输出参数：无
        // 返回值：生成的指令
        //----------------------------------------------------------------------
        private void _GetInstructionData(VisionSystemCommunicationLibrary.Ethernet.DataReceivedEventArgs ServerData, ref VisionSystemClassLibrary.Enum.CameraType Cameratype, ref Boolean bViewToolGraphics, ref Double dImageScale, ref Int32 iRejectImageIndex, ref Int32 iRejectImageNumber, ref Byte[] byteRejectListImage, ref Int32 iImageDataLength, ref VisionSystemClassLibrary.Struct.ImageInformation GraphicsInformation, ref MemoryStream memoryStream, ref Int32 iImageWidth, ref Int32 iImageHeight)
        {
            //指令类型 + 相机类型数据 + 图像工具数据（1，包含工具；0，不包含工具） + 图像尺寸类型数据 + 图像宽度数据 + 图像高度数据 + 剔除图像索引值数据 + 剔除图像个数数据 + 剔除图像名称索引值数据 + 图像信息数据 + 图像数据
            ...
            iRejectImageIndex = BitConverter.ToInt32(serializableData_1.Data_5, 0);//图像索引值数据

            iRejectImageNumber = BitConverter.ToInt32(serializableData_1.Data_6, 0);//剔除图像个数数据

            if (0 < iRejectImageNumber)//存在剔除图像
            {
                byteRejectListImage = new Byte[serializableData_1.Data_7.Length];

                serializableData_1.Data_7.CopyTo(byteRejectListImage, 0);//工具名称索引值数据
            }
        ...
        // 输入参数：1.InstructionType：指令类型
        //         2.Cameratype：相机类型
        //         3.iStatisticsDataType：统计数据类型（0，最新统计数据；1，指定统计数据）
        //         4.iShiftIndex：班次索引（从0开始）
        //         5.shifttime：统计数据开始结束时间
        // 输出参数：无
        // 返回值：生成的指令
        //----------------------------------------------------------------------
        private Byte[] _GenerateInstruction(CommunicationInstructionType InstructionType, VisionSystemClassLibrary.Enum.CameraType Cameratype, Int32 iStatisticsDataType, Int32 iShiftIndex, VisionSystemClassLibrary.Struct.ShiftTime shifttime)
        {
            //指令类型 + 相机类型数据 + 统计数据类型（0，最新统计数据；1，指定统计数据） + 班次索引（从0开始） + 统计数据开始结束时间
        ...
        // 返回值：生成的指令
        //----------------------------------------------------------------------
        private void _GetInstructionData(VisionSystemCommunicationLibrary.Ethernet.DataReceivedEventArgs ServerData, ref VisionSystemClassLibrary.Enum.CameraType Cameratype, ref Int32 iStatisticsDataType, ref Int32 iShiftIndex, ref VisionSystemClassLibrary.Struct.StatisticsInformation statisticsinformation)
        {
            //指令类型 + 相机类型数据 + 统计数据类型（0，当前班；1，历史班） + 班次索引（从0开始） + 统计数据开始结束时间 + 统计数据（品牌名称（包括品牌长度） + 已检测数量 + 合格数量 + 剔除数量 + 工具数量 + 工具统计信息）
        ...
        // 输入参数：1.InstructionType：指令类型
        //         2.Cameratype：相机类型
        //         3.iShiftIndex：班次索引（从0开始）
        //         4.shifttime：统计数据开始结束时间
        //         5.iToolIndex：剔除图像对应的工具索引值（从0开始）
        //         6.iImageIndex：剔除图像对应的工具中的索引值（从0开始）
        //         7.dImageScale：图像尺寸类型数据
        // 输出参数：无
        // 返回值：生成的指令
        //----------------------------------------------------------------------
        private Byte[] _GenerateInstruction(CommunicationInstructionType InstructionType, VisionSystemClassLibrary.Enum.CameraType Cameratype, Int32 iShiftIndex, VisionSystemClassLibrary.Struct.ShiftTime shifttime, Int32 iToolIndex, Int32 iImageIndex, Double dImageScale)
        {
            //指令类型 + 相机类型数据 + 班次索引（从0开始） + 统计数据开始结束时间 + 剔除图像对应的工具索引值（从0开始） + 剔除图像对应的工具中的索引值（从0开始） + 图像工具数据（1，包含工具；0，不包含工具） + 图像尺寸类型数据
        ...
        // 返回值：生成的指令
        //----------------------------------------------------------------------
        private void _GetInstructionData(VisionSystemCommunicationLibrary.Ethernet.DataReceivedEventArgs ServerData, ref VisionSystemClassLibrary.Enum.CameraType Cameratype, ref Int32 iShiftIndex, ref VisionSystemClassLibrary.Struct.ShiftTime shifttime, ref Int32 iToolIndex, ref Int32 iImageIndex, ref Double dImageScale, ref Int32 iImageDataLength, ref VisionSystemClassLibrary.Struct.ImageInformation GraphicsInformation, ref Image<Bgr, Byte> imageData)
        {
            //指令类型 + 相机类型数据 + 班次索引（从0开始） + 统计数据开始结束时间 + 剔除图像对应的工具索引值（从0开始） + 剔除图像对应的工具中的索引值（从0开始） + 图像尺寸类型数据 + 图像宽度数据 + 图像高度数据 + 图像信息数据 + 图像数据
