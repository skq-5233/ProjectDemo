...
namespace VisionSystemControlLibrary
{
    public partial class StatisticsControl : UserControl
    {
        ...
        private bool bRelevancy = false;//属性，【VIEW TOOL GRAPHICS】按钮状态。true：按钮按下；false：按钮未按下
        ...
        // 返回值：无
        //----------------------------------------------------------------------
        public void _Properties(VisionSystemClassLibrary.Class.Camera camera_parameter, VisionSystemClassLibrary.Class.Shift shift_parameter)
        {
                    ...
                    if (imageDisplayView.ControlSize.Width <= camera.ImageLearn.Width && imageDisplayView.ControlSize.Height <= camera.ImageLearn.Height)//有效
                    {
                        ...
                        if (bClickSelectAllButton)//选择所有
                        {
                            ...
                            for (i = 0; i < camera.Tools.Count; i++)
                            {
                                ...
                                {
                                    imageShow.Draw(new Rectangle(camera.Tools[i].WorkAreaPointX, camera.Tools[i].WorkAreaPointY, camera.Tools[i].WorkAreaWidth, camera.Tools[i].WorkAreaHeight), new Bgr(0, 255, 0), 1);
                                }
                            }
                        } 
                        else//选择当前
                        {
                            imageShow.Draw(new Rectangle(camera.Tools[customListRejects.CurrentListIndex].WorkAreaPointX, camera.Tools[customListRejects.CurrentListIndex].WorkAreaPointY, camera.Tools[customListRejects.CurrentListIndex].WorkAreaWidth, camera.Tools[customListRejects.CurrentListIndex].WorkAreaHeight), new Bgr(0, 255, 0), 1);
                        }
