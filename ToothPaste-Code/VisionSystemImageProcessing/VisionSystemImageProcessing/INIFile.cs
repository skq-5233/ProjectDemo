...
namespace VisionSystemImageProcessing
{
    public class INIFile
    {
        ...
        [DllImport("kernel32.dll", CharSet = CharSet.Auto)]//使用StringBuilder作为缓冲区类型的缺点是不能接受\0字符，会将\0及其后的字符截断,所以对于lpAppName或lpKeyName为null的情况就不适用
        ...
        {
            uint MAX_BUFFER = 32767;    //默认为3276799
        ...
        public static string[] GetSectionAllKeysAndValues(string section, string filePath)
        {
        ...
        /// <param name="iniFile">INI文件完整路径</param>
        /// <returns>如果没有内容,返回string[0]</returns>
        public static string[] GetSectionAllKeysOnly(string section, string filePath)
        {
        ...
        /// <returns>操作是否成功</returns>
        public static bool EmptySection(string section, string filePath)
        {
            return WritePrivateProfileSection(section, string.Empty, filePath);
        }
