...
namespace VisionSystemClassLibrary.Class
{
    ...
    public class Grid
    {
        ...
        public Struct.ROI rROI;//定义结构体，传输兴趣区域信息
        private Struct.ROI ROITemp;//定义结构体，向格子传输工作区域信息

        private Struct.Arithmetic aArithmetic = new Struct.Arithmetic();//算法结构体
        ...
        public Grid()
        {
            ...
            {
                aArithmetic.Number = Convert.ToByte(String.TextData.ArithmeticName_Grid_CHN.Length);
            }
            ...
            if ((image != null) && (minRectangle.Right <= image.Width) && (minRectangle.Bottom <= image.Height) && (minRectangle.Width > 0) && (minRectangle.Height > 0))
            {
                ...
                for (int j = 0; j < iRecicleExtra; j++)
                {
                    for (int i = 0; i < iRecicleInner; i++)
                    {
                        ...
                        {
                            Int32 iSearchHeight = j * cellSize + vOffset + remainderH;
                            Int32 iSearchHeightOffset = cellSize + iSearchHeight;
                            if ((iSearchHeight < 0) || (iSearchHeightOffset > ImageIntegral.Height)) //未超出图像高度
                            {
                                continue;
                            ...
                            {
                                Int32 iSearchWidth = i * cellSize + hOffset + remainderW;
                                Int32 iSearchWidthOffset = cellSize + iSearchWidth;
                                if ((iSearchWidth < 0) || (iSearchWidthOffset > ImageIntegral.Width)) //未超出图像宽度
                                {
                                    continue;
        ...
        //-----------------------------------------------------------------------
        // 功能说明：Grid类拷贝函数
        // 输入参数：1.Grid：grid，Grid参数
        // 输出参数：无
        // 返 回 值：无
        //----------------------------------------------------------------------
        public void _CopyTo(ref Grid grid)
        {
            grid.uiMaxDifferance = uiMaxDifferance;
            grid.cellSize = cellSize;
            grid.cellSizeTemp = cellSizeTemp;
            grid.hSearch = hSearch;
            grid.vSearch = vSearch;
            grid.iMin = iMin;
            grid.iMax = iMax;
            grid.color=color;
            ...
            if (null != CellLocation)
            {
                grid.CellLocation = new Point[CellLocation.GetLength(0), CellLocation.GetLength(1)];
                Array.Copy(CellLocation, grid.CellLocation, CellLocation.LongLength);
            }

            if (null != CellIntensity_Sample)
            {
                grid.CellIntensity_Sample = new UInt16[CellIntensity_Sample.GetLength(0), CellIntensity_Sample.GetLength(1)];
                Array.Copy(CellIntensity_Sample, grid.CellIntensity_Sample, CellIntensity_Sample.LongLength);
            }

            if (null != CellIntensity_Current)
            {
                grid.CellIntensity_Current = new UInt16[CellIntensity_Current.GetLength(0), CellIntensity_Current.GetLength(1)];
                Array.Copy(CellIntensity_Current, grid.CellIntensity_Current, CellIntensity_Current.LongLength);
            }

            if (null != CellCorrectState)
            {
                grid.CellCorrectState = new Boolean[CellCorrectState.GetLength(0), CellCorrectState.GetLength(1)];
                Array.Copy(CellCorrectState, grid.CellCorrectState, CellCorrectState.LongLength);
            }

            rROI._CopyTo(ref grid.rROI);
            ROITemp._CopyTo(ref grid.ROITemp);
            aArithmetic._CopyTo(ref grid.aArithmetic);

            grid.minRectangle = minRectangle;
        }
