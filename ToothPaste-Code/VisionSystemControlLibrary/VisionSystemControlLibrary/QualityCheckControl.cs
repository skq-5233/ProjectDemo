...
namespace VisionSystemControlLibrary
{
    public partial class QualityCheckControl : UserControl
    {
        ...
        private Boolean bSettingInnerROISelected = false;

        //

        private Int32 iRejectImageMaxNumber = 0;//剔除图像最大值

        private Int32 iRejectImageIndex = -1;//属性（只读）

        //

        private Int32 iSelectedShiftIndex = 0;//属性
        private VisionSystemClassLibrary.Struct.ShiftTime shifttimeSelected = new VisionSystemClassLibrary.Struct.ShiftTime();//属性
        ...
        private VisionSystemClassLibrary.Struct.ROI drawROI;//绘图兴趣区域
        private VisionSystemClassLibrary.Struct.ROI drawROIOriginal;//原始绘图兴趣区域
        ...
        public QualityCheckControl()
        {
            ...
            labelROIXYValue.Text = "";
            labelROIWHValue.Text = "";
            ...
            drawROI = new VisionSystemClassLibrary.Struct.ROI();
            drawROIOriginal = new VisionSystemClassLibrary.Struct.ROI();
        ...
        //----------------------------------------------------------------------
        // 功能说明：RejectImageMaxNumber属性的实现
        // 输入参数：无
        // 输出参数：无
        // 返回值：无
        //----------------------------------------------------------------------
        [Browsable(false), Description("剔除图像最大值"), Category("QualityCheckControl 通用")]
        public Int32 RejectImageMaxNumber//属性
        {
            get//读取
            {
                return iRejectImageMaxNumber;
            }
        }
        ...
        // 返 回 值：无
        //----------------------------------------------------------------------
        [Browsable(false), Description("班次索引"), Category("QualityCheckControl 通用")]
        public Int32 SelectedShiftIndex
        {
            get//读取
            {
                return iSelectedShiftIndex;
            }
        }
        ...
        //-----------------------------------------------------------------------
        // 功能说明：shifttimeSelected属性
        // 输入参数：无
        // 输出参数：无
        // 返 回 值：无
        //----------------------------------------------------------------------
        [Browsable(false), Description("班次开始结束时间"), Category("QualityCheckControl 通用")]
        public VisionSystemClassLibrary.Struct.ShiftTime ShiftTimeSelected
        {
            get//读取
            {
                return shifttimeSelected;
            }
        }
        ...
        // 返回值：无
        //----------------------------------------------------------------------
        [Browsable(true), Description("【Relevancy】按钮状态。true：按钮按下；false：按钮未按下"), Category("QualityCheckControl 通用")]
        ...
        private void _SetCamera()
        {
            ...
            bSettingInnerROISelected = false;
            customButtonEditTools.CustomButtonBackgroundImage = CustomButton_BackgroundImage.Disable;//【EDIT TOOLS】
            customButtonEditTools.Chinese_TextDisplay = new String[1] { "设置&外部区域" };//设置显示的文本
            customButtonEditTools.English_TextDisplay = new String[1] { "SET&Extra ROI" };//设置显示的文本
            ...
            iRejectImageIndex = iRejectImageMaxNumber - 1;
        ...
        private void _SetToolParameters()
        {
            try
            {
                if (iToolParameterNumber[iCurrentToolIndex] > 0) //参数需要显示
                {
                    Int32 i = 0;//循环控制变量
                    Int32 j = 0;//循环控制变量
        ...
        private void _SetLanguage()
        {
            ...
            customButtonEditTools.Language = language;
        ...
        private void _Close()
        {
            ...
            bSettingInnerROISelected = false;
            customButtonEditTools.CustomButtonBackgroundImage = CustomButton_BackgroundImage.Up;
            customButtonEditTools.Chinese_TextDisplay = new String[1] { "设置&外部区域" };//设置显示的文本
            customButtonEditTools.English_TextDisplay = new String[1] { "SET&Extra ROI" };//设置显示的文本
        ...
        public void _LearnSample(Boolean bSuccess, VisionSystemClassLibrary.Struct.ImageInformation imageInformation)
        {
            if (bSuccess)//成功
            {
                ...
                customButtonSaveProduct.BitmapWhole = Properties.Resources.Button_1;//【SAVE PRODUCT】
        ...
        private void _GetImage()
        {
            switch (iImageType)
            {
                ...
                default://3，显示处理图像来自剔除图像
                    ...
                    {
                        if (camera.Rejects.GraphicsInformation.Scale >= 1.0)
                        {
        ...
        //-----------------------------------------------------------------------
        // 功能说明：刷新界面函数
        // 输入参数：1.relevancy：相机关联标记
        //         2.iShiftIndex：班次索引
        //         3.shiftTime：班次起止时间
        //         3.iRejectImageTotal：剔除图像总数
        // 输出参数：无
        // 返 回 值：无
        //----------------------------------------------------------------------
        public void _UpdateShiftInfo(Boolean relevancy, Int32 iShiftIndex, VisionSystemClassLibrary.Struct.ShiftTime shiftTime, Int32 iRejectImageTotal)
        {
            bRelevancy = relevancy;

            iRejectImageMaxNumber = iRejectImageTotal;
            iRejectImageIndex = iRejectImageMaxNumber - 1;

            iSelectedShiftIndex = iShiftIndex;

            shifttimeSelected = shiftTime;
        ...
        // 返 回 值：无
        //----------------------------------------------------------------------
        public void _UpdateInterface(VisionSystemClassLibrary.Struct.ImageInformation imageInformation, Int32 iImageType_Temp = -1)
        {
        ...
        private void _SelectTool()
        {
            ...
            if (camera.Tools[iCurrentToolIndex].ROI.roiInnerExit) //存在内轮廓
            {
                customButtonEditTools.Visible = true;
            }
            else
            {
                customButtonEditTools.Visible = false;
                bSettingInnerROISelected = false;
                customButtonEditTools.CustomButtonBackgroundImage = CustomButton_BackgroundImage.Up;
                customButtonEditTools.Chinese_TextDisplay = new String[1] { "设置&外部区域" };//设置显示的文本
                customButtonEditTools.English_TextDisplay = new String[1] { "SET&Extra ROI" };//设置显示的文本
            ...
            camera.Tools[iCurrentToolIndex].ROI._CopyTo(ref drawROI);
        ...
        // 返 回 值： 无
        //----------------------------------------------------------------------
        private void _HandleLocation(VisionSystemClassLibrary.Struct.ROI rect)
        {
            Rectangle rectangle = new Rectangle();
            VisionSystemClassLibrary.Struct.ROI_Inner roi_Inner = rect.roiExtra;
            if (bSettingInnerROISelected) //设置内部区域
            {
                roi_Inner = rect.roiInner;
            }

            switch (roi_Inner.roiType)
            {
                case VisionSystemClassLibrary.Enum.ROIType.Quadrangle:
                    labelHandleLeftTop.Location = new Point(imageDisplayView.Location.X + (Int32)(roi_Inner.Point1.X / imageDisplayView.ImageScale), imageDisplayView.Location.Y + imageDisplayView.RectangleImage.Location.Y + (Int32)(roi_Inner.Point1.Y / imageDisplayView.ImageScale));
                    labelHandleRightTop.Location = new Point(imageDisplayView.Location.X + (Int32)(roi_Inner.Point2.X / imageDisplayView.ImageScale), imageDisplayView.Location.Y + imageDisplayView.RectangleImage.Location.Y + (Int32)(roi_Inner.Point2.Y / imageDisplayView.ImageScale));
                    labelHandleRightBottom.Location = new Point(imageDisplayView.Location.X + (Int32)(roi_Inner.Point3.X / imageDisplayView.ImageScale), imageDisplayView.Location.Y + imageDisplayView.RectangleImage.Location.Y + (Int32)(roi_Inner.Point3.Y / imageDisplayView.ImageScale));
                    labelHandleLeftBottom.Location = new Point(imageDisplayView.Location.X + (Int32)(roi_Inner.Point4.X / imageDisplayView.ImageScale), imageDisplayView.Location.Y + imageDisplayView.RectangleImage.Location.Y + (Int32)(roi_Inner.Point4.Y / imageDisplayView.ImageScale));
                    break;
                case VisionSystemClassLibrary.Enum.ROIType.Ellipse:
                    rectangle = VisionSystemClassLibrary.GeneralFunction._GetMinRect(roi_Inner);

                    rectangle.X = imageDisplayView.Location.X + (Int32)(rectangle.X / imageDisplayView.ImageScale);
                    rectangle.Y = imageDisplayView.Location.Y + imageDisplayView.RectangleImage.Location.Y + (Int32)(rectangle.Y / imageDisplayView.ImageScale);
                    rectangle.Width = (Int32)(rectangle.Width / imageDisplayView.ImageScale);
                    rectangle.Height = (Int32)(rectangle.Height / imageDisplayView.ImageScale);
        ...
        {
            labelHandleLeftTop.BackColor = Color.Blue;
            labelHandleTop.BackColor = Color.Blue;
            labelHandleRightTop.BackColor = Color.Blue;
            labelHandleLeft.BackColor = Color.Blue;
            labelHandleRight.BackColor = Color.Blue;
            labelHandleLeftBottom.BackColor = Color.Blue;
            labelHandleBottom.BackColor = Color.Blue;
            labelHandleRightBottom.BackColor = Color.Blue;
        ...
        private void _ShowHandle(Boolean bVisible, VisionSystemClassLibrary.Struct.ROI rect)
        {
            bDrawImageProcessingResult = true;
        ...
        {
            VisionSystemClassLibrary.Struct.ROI_Inner roi_Inner = drawROI.roiExtra;

            if ((!bHandleVisible) && (!bSizeAdjustedPanelVisible))//手柄HIDE，控制板HIDE
            {
                if (_IsContain(roi_Inner, pointValue))//单击轮廓内部
                {
                    ...
                    drawROI._CopyTo(ref drawROIOriginal);
                }
                else//单击轮廓外部
                {
                    //不执行操作
            ...
            {
                if (false == _IsContain(roi_Inner, pointValue))//单击轮廓外部
                {
                    _ShowHandle(false, drawROI);
                    _ResetHandleColor();
                }
            ...
            else if (bHandleVisible && bSizeAdjustedPanelVisible)//手柄SHOW，控制板SHOW
            {
                ...
                else//单击轮廓内部
                {
                    ...
                    sizeAdjustedPanel.SelectionType = RegionSelectionType.None;
                }
        ...
        private Boolean _CheckArithmetic_Angle()
        {
            ...
            if (VisionSystemClassLibrary.Enum.ArithmeticType.Ruler == camera.Tools[iCurrentToolIndex].Type)//直尺
            {
        ...
        private void _UpdateROI()
        {
            ...
            if (false == bMeasureToolSelected) //
            {
                VisionSystemClassLibrary.Struct.ROI_Inner roiInner = drawROI.roiExtra;
                if (bSettingInnerROISelected) //操作内部区域
                {
                    roiInner = drawROI.roiInner;
                }
                Rectangle rect = VisionSystemClassLibrary.GeneralFunction._GetMinRect(roiInner);
                labelROIXYValue.Text = rect.Left.ToString() + "，" + rect.Top.ToString();//设置显示的文本
                labelROIWHValue.Text = rect.Width.ToString() + "，" + rect.Height.ToString();//设置显示的文本
            }
        ...
        private void _ROIChange()
        {
            ...
            VisionSystemClassLibrary.Struct.ROI rectangleROI_Temp = new VisionSystemClassLibrary.Struct.ROI();//兴趣区域临时变量
            drawROI._CopyTo(ref rectangleROI_Temp);
            ...
            if (bHandleVisible)//显示
            {
                    ...
                    case VisionSystemClassLibrary.Enum.ROIType.Quadrangle:
                        switch (contentalignmentHandleType)
                        {
                            case ContentAlignment.TopLeft://拖动手柄上左按钮
                                ...
                                roi_Inner.Point1 = pointMouse;

                                if (_OperateIsOK(rectangleImage_Temp, rectangleROI_Temp, bSettingInnerROISelected, true)) //满足条件
                                {
                                    bOperateResult = true;
                                }
                                break;
                        ...
                        break;
                    case VisionSystemClassLibrary.Enum.ROIType.Ellipse:
                        VisionSystemClassLibrary.Struct.ROI_Inner roi_Inner_Original = drawROIOriginal.roiExtra;
                        if (bSettingInnerROISelected) //设置内部区域
                        {
                            roi_Inner_Original = drawROIOriginal.roiInner;
                        }
                        ...
                        switch (contentalignmentHandleType)
                        {
                            case ContentAlignment.TopCenter://拖动手柄上中按钮
                                ...
                                pointMouse.Y = (Int32)((Control.MousePosition.Y - pointMouse_Move.Y - pointImageDisplayControl.Y - imageDisplayView.RectangleImage.Location.Y) * imageDisplayView.ImageScale);
                                roi_Inner.Point2.Y = roi_Inner_Original.Point1.Y - pointMouse.Y;
                            ...
                            case ContentAlignment.MiddleLeft://拖动手柄中左按钮
                                ...
                                pointMouse.X = (Int32)((Control.MousePosition.X - pointMouse_Move.X - pointImageDisplayControl.X) * imageDisplayView.ImageScale);
                                roi_Inner.Point2.X = roi_Inner_Original.Point1.X - pointMouse.X;
                            ...
                            case ContentAlignment.MiddleRight://拖动手柄中右按钮
                                ...
                                pointMouse.X = (Int32)((Control.MousePosition.X + pointMouse_Move.X - pointImageDisplayControl.X) * imageDisplayView.ImageScale);
                                roi_Inner.Point2.X = pointMouse.X - roi_Inner_Original.Point1.X;
                            ...
                            case ContentAlignment.BottomCenter://拖动手柄下中按钮
                                ...
                                pointMouse.Y = (Int32)((Control.MousePosition.Y + pointMouse_Move.Y - pointImageDisplayControl.Y - imageDisplayView.RectangleImage.Location.Y) * imageDisplayView.ImageScale);
                                roi_Inner.Point2.Y = pointMouse.Y - roi_Inner_Original.Point1.Y;
                        ...
                        break;
                    case VisionSystemClassLibrary.Enum.ROIType.Rectangle:
                        Rectangle rectROIOriginal;
                        if (bSettingInnerROISelected) //设置内部区域
                        {
                            rectROIOriginal = VisionSystemClassLibrary.GeneralFunction._GetMinRect(drawROIOriginal.roiInner);
                        }
                        else
                        {
                            rectROIOriginal = VisionSystemClassLibrary.GeneralFunction._GetMinRect(drawROIOriginal.roiExtra);
                        }
                        ...
                        switch (contentalignmentHandleType)
                        {
                            case ContentAlignment.TopCenter://拖动手柄上中按钮
                                ...
                                pointMouse.Y = (Int32)((Control.MousePosition.Y - pointMouse_Move.Y - pointImageDisplayControl.Y - imageDisplayView.RectangleImage.Location.Y) * imageDisplayView.ImageScale);
                                roi_Inner.Point1.Y = pointMouse.Y;
                                roi_Inner.Point2.Y = rectROIOriginal.Bottom - pointMouse.Y;
                            ...
                            case ContentAlignment.MiddleLeft://拖动手柄中左按钮
                                ...
                                pointMouse.X = (Int32)((Control.MousePosition.X - pointMouse_Move.X - pointImageDisplayControl.X) * imageDisplayView.ImageScale);
                                roi_Inner.Point1.X = pointMouse.X;
                                roi_Inner.Point2.X = rectROIOriginal.Right - pointMouse.X;
                            ...
                            case ContentAlignment.MiddleRight://拖动手柄中右按钮
                                ...
                                pointMouse.X = (Int32)((Control.MousePosition.X + pointMouse_Move.X - pointImageDisplayControl.X) * imageDisplayView.ImageScale);
                                roi_Inner.Point2.X = pointMouse.X - rectROIOriginal.Left;
                            ...
                            case ContentAlignment.BottomCenter://拖动手柄下中按钮
                                ...
                                pointMouse.Y = (Int32)((Control.MousePosition.Y + pointMouse_Move.Y - pointImageDisplayControl.Y - imageDisplayView.RectangleImage.Location.Y) * imageDisplayView.ImageScale);
                                roi_Inner.Point2.Y = pointMouse.Y - rectROIOriginal.Top;
                ...
                {
                    rectangleROI_Temp._CopyTo(ref drawROI);

                    //

                    drawROI.roiExtra._CopyTo(ref camera.Tools[iCurrentToolIndex].ROI.roiExtra);
                    drawROI.roiInner._CopyTo(ref camera.Tools[iCurrentToolIndex].ROI.roiInner);
                    camera.Tools[iCurrentToolIndex].ROI.roiInnerExit = drawROI.roiInnerExit;
        ...
        private void _SetSizeAdjustedPanel()
        {
            ...
            sizeAdjustedPanel.HandleType = contentalignmentHandleType;
            sizeAdjustedPanel.SettingInnerROI = bSettingInnerROISelected;
        }
        ...
        private void customButtonManageTools_CustomButton_Click(object sender, EventArgs e)
        {
            ...
            else//未按下
            {
                ...
                if (bSettingInnerROISelected)//按下
                {
                    customButtonEditTools.CustomButtonBackgroundImage = CustomButton_BackgroundImage.Selected;//【EDIT TOOLS
                }
                else
                {
                    customButtonEditTools.CustomButtonBackgroundImage = CustomButton_BackgroundImage.Up;//【EDIT TOOLS
        ...
        {
            bSettingInnerROISelected = !bSettingInnerROISelected;

            if (bSettingInnerROISelected) //设置内轮廓
            {
                customButtonEditTools.Chinese_TextDisplay = new String[1] { "设置&内部区域" };//设置显示的文本
                customButtonEditTools.English_TextDisplay = new String[1] { "SET&Inner ROI" };//设置显示的文本
            }
            else //设置外轮廓
            {
                customButtonEditTools.Chinese_TextDisplay = new String[1] { "设置&外部区域" };//设置显示的文本
            ...
            if (bSizeAdjustedPanelVisible)//控制板可见
            {
        ...
        {
            if (iRejectImageIndex < iRejectImageMaxNumber - 1)
            {
                iRejectImageIndex++;
            }
            else
        ...
        private void customButtonMeasureTool_CustomButton_Click(object sender, EventArgs e)
        {
            ...
            if (bMeasureToolSelected)//按下
            {
                labelROIXY.Visible = false;
                labelROIXYValue.Visible = false;
                labelROIWH.Visible = false;
                labelROIWHValue.Visible = false;
            ...
            else//未按下
            {
                ...
                labelROIXY.Visible = true;
                labelROIXYValue.Visible = true;
                labelROIWH.Visible = true;
                labelROIWHValue.Visible = true;
            }
        ...
        private void imageDisplayView_Control_DoubleClick(object sender, EventArgs e)
        {
            if (!bManageToolsSelected)//【MANAGE TOOLS】未按下
            {
                ...
                {
                    if (_IsContain(drawROI.roiExtra, pointValue))//双击轮廓内部
                    {
                        sizeAdjustedPanel.SelectionType = RegionSelectionType.None;
        ...
        private void imageDisplayView_Control_MouseDown(object sender, EventArgs e)
        {
            if (!bManageToolsSelected)//【MANAGE TOOLS】未按下
            {
                ...
                Rectangle rect = VisionSystemClassLibrary.GeneralFunction._GetMinRect(drawROI.roiExtra);
                pointMouse_Move.X = (Int32)((pointValue.X - rect.X) / imageDisplayView.ImageScale);
                pointMouse_Move.Y = (Int32)((pointValue.Y - rect.Y) / imageDisplayView.ImageScale);

                //

                if (_IsContain(drawROI.roiExtra, pointValue))//单击轮廓内部
                {
        ...
        /// <returns></returns>
        private Boolean _IsContain(VisionSystemClassLibrary.Struct.ROI_Inner inner, Point point)
        {
            Boolean result = false;

            switch (inner.roiType)
            {
                case VisionSystemClassLibrary.Enum.ROIType.Quadrangle:
                    Point[] points = new Point[4];
                    points[0] = inner.Point1;
                    points[1] = inner.Point2;
                    points[2] = inner.Point3;
                    points[3] = inner.Point4;

                    if (_Contains(points, point))
                    {
                        result = true;
                    }
                    break;
                case VisionSystemClassLibrary.Enum.ROIType.Ellipse:
                    if ((Math.Pow((point.X - inner.Point1.X), 2) / Math.Pow(inner.Point2.X, 2) + Math.Pow((point.Y - inner.Point1.Y), 2) / Math.Pow(inner.Point2.Y, 2)) < 1)
                    {
                        result = true;
                    }
                    break;
                default:
                    Rectangle rect = new Rectangle(inner.Point1, new Size(inner.Point2));
        ...
        /// <returns></returns>
        public bool _Contains(Point[] points, Point test)
        {
            int i, j;
            bool result = false;
        ...
        private void imageDisplayView_Control_MouseMove(object sender, EventArgs e)
        {
            if (!bManageToolsSelected && bMouseMove_MouseIn)//【MANAGE TOOLS】未按下
            {
                ...
                if (bHandleVisible || sizeAdjustedPanel.Visible)//手柄可见
                {
                    ...
                    Int32 pointX = (Int32)((Control.MousePosition.X - pointMouse_Move.X - pointImageDisplayControl.X) * imageDisplayView.ImageScale);
                    Int32 pointY = (Int32)((Control.MousePosition.Y - pointMouse_Move.Y - pointImageDisplayControl.Y - imageDisplayView.RectangleImage.Location.Y) * imageDisplayView.ImageScale);

                    Rectangle rect = VisionSystemClassLibrary.GeneralFunction._GetMinRect(rectangleROI_Temp.roiExtra);
                    rectangleROI_Temp._Offset(pointX - rect.X, pointY - rect.Y);

                    if (_OperateIsOK(rectangleImage_Temp, rectangleROI_Temp, bSettingInnerROISelected, false)) //满足条件
                    {
                        rectangleROI_Temp._CopyTo(ref drawROI);
                        ...
                        drawROI.roiExtra._CopyTo(ref sizeAdjustedPanel.RectangleROI.roiExtra);
                        drawROI.roiInner._CopyTo(ref sizeAdjustedPanel.RectangleROI.roiInner);
                        sizeAdjustedPanel.RectangleROI.roiInnerExit = drawROI.roiInnerExit;
                ...
                if (bMouseMove)
                {
                    ...
                    if (bHandleVisible || sizeAdjustedPanel.Visible)//手柄可见
                    {
                        ...
                        if (_OperateIsOK(rectangleImage_Temp, rectangleROI_Temp, bSettingInnerROISelected, false)) //满足条件
                        {
                            ...
                            customButtonSaveProduct.BitmapWhole = Properties.Resources.Button_1;
        ...
        private void sizeAdjustedPanel_Left_Click(object sender, EventArgs e)
        {
            ...
            sizeAdjustedPanel.RectangleROI._CopyTo(ref drawROI);
        ...
        {
            _ShowHandle(!sizeAdjustedPanel.ShowHideButton, drawROI);
        }
        ...
        private void labelHandleTop_MouseDown(object sender, MouseEventArgs e)
        {
            ...
            sizeAdjustedPanel.SelectionType = RegionSelectionType.Top;
        ...
        private void labelHandleRightTop_MouseDown(object sender, MouseEventArgs e)
        {
            ...
            sizeAdjustedPanel.SelectionType = RegionSelectionType.RightTop;
        ...
        private void labelHandleLeft_MouseDown(object sender, MouseEventArgs e)
        {
            ...
            sizeAdjustedPanel.SelectionType = RegionSelectionType.Left;
        ...
        private void labelHandleRight_MouseDown(object sender, MouseEventArgs e)
        {
            ...
            sizeAdjustedPanel.SelectionType = RegionSelectionType.Right;
        ...
        private void labelHandleLeftBottom_MouseDown(object sender, MouseEventArgs e)
        {
            ...
            sizeAdjustedPanel.SelectionType = RegionSelectionType.LeftBottom;
        ...
        private void labelHandleBottom_MouseDown(object sender, MouseEventArgs e)
        {
            ...
            sizeAdjustedPanel.SelectionType = RegionSelectionType.Bottom;
        ...
        private void labelHandleRightBottom_MouseDown(object sender, MouseEventArgs e)
        {
            ...
            sizeAdjustedPanel.SelectionType = RegionSelectionType.RightBottom;
        ...
        /// <summary>
        /// 判断是否是凸四边形，且点3和点4在点1、2的同侧，点1和点4在点2、3的同侧
        /// </summary>
        /// <returns></returns>
        public static Boolean _IsConvexQuadrangle(Point point1, Point point2, Point point3, Point point4)
        {
            //if ((point4.Y - point3.Y) * (point4.X - point2.X) == (point4.Y - point2.Y) * (point4.X - point3.X))
            //    return false;
            //else if ((point4.Y - point3.Y) * (point4.X - point1.X) == (point4.Y - point1.Y) * (point4.X - point3.X))
            //    return false;
            //else if ((point4.Y - point2.Y) * (point4.X - point1.X) == (point4.Y - point1.Y) * (point4.X - point2.X))
            //    return false;
            //else if ((point3.Y - point2.Y) * (point3.X - point1.X) == (point3.Y - point1.Y) * (point3.X - point2.X))
            //    return false;           //任意三个顶点成直线，非四边形
            //else
            //    return true;

            double z1, z2, z3, z4, z5, z6;
            z1 = ((point4.X - point1.X) * (point2.Y - point1.Y) - (point2.X - point1.X) * (point4.Y - point1.Y));
            z2 = ((point1.X - point2.X) * (point3.Y - point2.Y) - (point3.X - point2.X) * (point1.Y - point2.Y));
            z3 = ((point2.X - point3.X) * (point4.Y - point3.Y) - (point4.X - point3.X) * (point2.Y - point3.Y));
            z4 = ((point3.X - point4.X) * (point1.Y - point4.Y) - (point1.X - point4.X) * (point3.Y - point4.Y));
            z5 = ((point3.X - point1.X) * (point2.Y - point1.Y) - (point2.X - point1.X) * (point3.Y - point1.Y));
            z6 = ((point4.X - point2.X) * (point3.Y - point2.Y) - (point3.X - point2.X) * (point4.Y - point2.Y));

            return ((z1 * z2 * z3 * z4 > 0) && (z1 * z5 > 0) && (z2 * z6 > 0));
        }
        ...
        /// </summary>
        /// <returns></returns>
        public static Boolean _IsEllipse(Point point2)
        {
            if ((point2.X > 0) && (point2.Y > 0)) //椭圆宽度和高度不为零
            {
                return true;
            }
            else
        ...
        /// </summary>
        /// <returns></returns>
        public static Boolean _IsRectangle(Point point2)
        {
            if ((point2.X > 0) && (point2.Y > 0)) //椭圆宽度和高度不为零
            {
                return true;
        ...
        /// 计算调整后区域是否符合要求
        /// </summary>
        /// <param name="rectangleMaxROI"></param>
        /// <param name="rectangleROI_Temp"></param>
        /// <param name="bTest"></param>
        /// <returns></returns>
        public static Boolean _OperateIsOK(Rectangle rectangleMaxROI, VisionSystemClassLibrary.Struct.ROI rectangleROI_Temp, Boolean bSettingInnerROISelected, Boolean bTest)
        {
            if (false == rectangleMaxROI.Contains(VisionSystemClassLibrary.GeneralFunction._GetMinRect(rectangleROI_Temp.roiExtra))) //外部区域在范围内
            {
                return false;
            }

            if (rectangleROI_Temp.roiInnerExit) //包含内部区域
            {
                if (false == VisionSystemClassLibrary.GeneralFunction._GetMinRect(rectangleROI_Temp.roiExtra).Contains(VisionSystemClassLibrary.GeneralFunction._GetMinRect(rectangleROI_Temp.roiInner)))
                {
                    return false;
                }
            }
            ...
            if (bTest) //测试是否满足形状
            {
                ...
                {
                    case VisionSystemClassLibrary.Enum.ROIType.Quadrangle:
                        bResult = _IsConvexQuadrangle(roi_Inner.Point1, roi_Inner.Point2, roi_Inner.Point3, roi_Inner.Point4);//测试是否满足形状
                        break;
                    case VisionSystemClassLibrary.Enum.ROIType.Ellipse:
                        bResult = _IsEllipse(roi_Inner.Point2);
                        break;
                    case VisionSystemClassLibrary.Enum.ROIType.Rectangle:
                        bResult = _IsRectangle(roi_Inner.Point2);
                        break;
                    default:
                        bResult = false;
                        break;
