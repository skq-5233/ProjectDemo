...
namespace VisionSystemControlLibrary
{
    public partial class TolerancesControl : UserControl
    {
        ...
        public void _SetEjectLevel(int iGraphIndex, Int32 iMin, Int32 iMax, Int32 iUpdateTolerances)
        {
            ...
            {
                if (VisionSystemClassLibrary.Enum.ArithmeticType.Tobacco_D == camera.Tools[camera.Tolerances.GraphData[iGraphIndex].ToolsIndex].Type)//更改公差下限，且执行烟支检测
                {
        ...
        private void _SetView()
        {
            ...
            {
                imageDisplayView.Information = camera.Rejects.GraphicsInformation;
            ...
            if (bUpperLower)//Y轴最大（有效）值
            {
                ...
                if (iNewValue >= tolerancesGraph.Control_Graph.Control_Data.TolerancesTools.MinValue && iNewValue <= tolerancesGraph.Control_Graph.Control_Data.TolerancesTools.MaxValue)//符合要求
                {
                    if (iNewValue >= tolerancesGraph.YAxisEffectiveMinValue)//符合要求
                    {
                        ...
                        if (VisionSystemClassLibrary.Enum.ArithmeticType.Tobacco_D == camera.Tools[camera.Tolerances.GraphData[iGraphDataIndex].ToolsIndex].Type)//更改公差上限，且执行烟支检测
                        {
                        ...
                        else if (camera.Tools[camera.Tolerances.GraphData[iGraphDataIndex].ToolsIndex].Detect_89713FA && (VisionSystemClassLibrary.Enum.ArithmeticType.BaleLoosing == camera.Tools[camera.Tolerances.GraphData[iGraphDataIndex].ToolsIndex].Type)) //更改公差上限，且执行烟支检测
                        {
                            camera.Tolerances._GetPrecision_Tobacco_D((Int16)iEjectLevel, iNewValue, camera.Tools[camera.Tolerances.GraphData[iGraphDataIndex].ToolsIndex].EjectPixelMin, ref iPrecision[camera.Tolerances.GraphData[iGraphDataIndex].ToolsIndex]);
                        }
                    }
            ...
            else//Y轴最小（有效）值
            {
                ...
                if (iNewValue >= tolerancesGraph.Control_Graph.Control_Data.TolerancesTools.MinValue && iNewValue <= tolerancesGraph.Control_Graph.Control_Data.TolerancesTools.MaxValue)//符合要求
                {
                    if (iNewValue <= tolerancesGraph.YAxisEffectiveMaxValue)//符合要求
                    {
                        ...
                        if (VisionSystemClassLibrary.Enum.ArithmeticType.Tobacco == camera.Tools[camera.Tolerances.GraphData[iGraphDataIndex].ToolsIndex].Type)//更改公差下限，且执行烟支检测
                        {
